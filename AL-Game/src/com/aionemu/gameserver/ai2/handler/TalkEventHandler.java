		...
		if (creature instanceof Player) {
			...
			if (QuestEngine.getInstance().onDialog(new QuestEnv(npcAI.getOwner(), player, 0, -1))) {
				...
				case 461178: //Studio Manager.
					...
					if (studio == null && house == null && ownerId == 0) {
						if (player.getRace() == Race.ELYOS) {
							QuestState qsE = player.getQuestStateList().getQuestState(18802);
							if (qsE.getStatus() == QuestStatus.COMPLETE) {
								PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(npcAI.getOwner().getObjectId(), 10));
								return;
							}
						} else if (player.getRace() == Race.ASMODIANS) {
							QuestState qsA = player.getQuestStateList().getQuestState(28802);
							if (qsA.getStatus() == QuestStatus.COMPLETE) {
								PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(npcAI.getOwner().getObjectId(), 10));
								return;
							}
						}
