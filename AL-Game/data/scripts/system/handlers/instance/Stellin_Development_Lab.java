...
@InstanceID(302550000)
public class Stellin_Development_Lab extends GeneralInstanceHandler
{
    ...
    public void onDropRegistered(Npc npc) {
		...
		switch (npcId) {
			...
			break;
			case 839632: //Daeva's Mutant Cubic Crystal.
			    for (Player player: instance.getPlayersInside()) {
				    if (player.isOnline()) {
						dropItems.add(DropRegistrationService.getInstance().regDropItem(index++, player.getObjectId(), npcId, 188072804, 1));
					}
				}
			break;
        }
    ...
    public void onEnterInstance(final Player player) {
		...
		if (player.getRace() == Race.ELYOS) {
			ClassChangeService.onUpdateQuest62820(player);
		} else {
			ClassChangeService.onUpdateQuest72820(player);
		}
    }
	...
	public void onDie(Npc npc) {
		...
		switch (npc.getObjectTemplate().getTemplateId()) {
			...
			break;
			case 858115: //Mutated Daeva.
				spawn(838086, 210.0000f, 275.0000f, 570.0000f, (byte) 90);
				spawn(839632, 211.0000f, 260.0000f, 570.0000f, (byte) 90); //Daeva's Mutant Cubic Crystal.
				instance.doOnAllPlayers(new Visitor<Player>() {
					@Override
					public void visit(Player player) {
						player.getController().updateZone();
