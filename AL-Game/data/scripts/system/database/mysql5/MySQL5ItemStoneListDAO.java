...
public class MySQL5ItemStoneListDAO extends ItemStoneListDAO
{
                ...
                if (item.getItemTemplate().isArmor() || item.getItemTemplate().isWeapon()) {
                    ...
                    while (rset.next()) {
                        ...
                        switch (stoneType) {
                            ...
                            break;
                            case 4:
                                item.setOdianStone(new OdianStone(item.getObjectId(), itemId, PersistentState.UPDATED));
                            break;
                            case 5:
                                item.setRuneStone(new RuneStone(item.getObjectId(), itemId, PersistentState.UPDATED));
                            break;
                        }
                    }
                    rset.close();
    ...
    public void save(List<Item> items) {
        ...
        Set<OdianStone> odianStones = Sets.newHashSet();
        Set<RuneStone> runeStones = Sets.newHashSet();
        for (Item item : items) {
            ...
            OdianStone odianStone = item.getOdianStone();
            if (odianStone != null) {
                odianStones.add(odianStone);
            }
            RuneStone runeStone = item.getRuneStone();
            if (runeStone != null) {
        ...
        store(odianStones, ItemStoneType.ODIANSTONE);
        store(runeStones, ItemStoneType.RUNESTONE);
    }
    ...
    @Override
    public void storeOdianStones(OdianStone odianStone) {
        store(Collections.singleton(odianStone), ItemStoneType.ODIANSTONE);
    }

    @Override
    public void storeRuneStones(RuneStone runeStone) {
        store(Collections.singleton(runeStone), ItemStoneType.RUNESTONE);
    }
