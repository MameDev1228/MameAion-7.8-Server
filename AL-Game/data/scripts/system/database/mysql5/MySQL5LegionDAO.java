...
public class MySQL5LegionDAO extends LegionDAO
{
	...
	public boolean isNameUsed(final String name) {
		PreparedStatement s = DB.prepareStatement("SELECT 1 FROM legions WHERE name = ? LIMIT 1");
		try {
			s.setString(1, name);
	...
	public LegionEmblem loadLegionEmblem(final int legionId) {
		...
		DB.select(SELECT_EMBLEM_QUERY, new ParamReadStH() {
			...
			public void handleRead(ResultSet resultSet) throws SQLException {
				while (resultSet.next()) {
					legionEmblem.setEmblem(resultSet.getInt("emblem_id"),
		...
		DB.select(SELECT_STORAGE_QUERY, new ParamReadStH() {
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					optionalFusionSocket, charge, randomNumber, rndCount, wrappingCount, isPacked == 1, temperingLevel, isTopped == 1, strengthenSkill, skinSkill, isLunaReskin == 1, reductionLevel, unSeal, isEnhance, enhanceSkillId, enhanceSkillEnchant, grindSocket, grindColor, contaminated, 0, 0);
