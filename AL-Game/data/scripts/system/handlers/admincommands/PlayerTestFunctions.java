...
public class PlayerTestFunctions extends AdminCommand
{
	...
	public void execute(Player player, String... params) {
		if (params.length == 0) {
			showHelp(player);
			return;
		} if (COMMAND_PROTECTOR.equalsIgnoreCase(params[0]) || COMMAND_CONQUEROR.equalsIgnoreCase(params[0]) || COMMAND_GOLDEN_STAR.equalsIgnoreCase(params[0])) {
			handleUpdate(player, params);
			player.clearKnownlist();
			PacketSendUtility.sendPacket(player, new SM_PLAYER_INFO(player, false));
			player.updateKnownlist();
		} else if (XP.equalsIgnoreCase(params[0]) || LUNA_RECIPE.equalsIgnoreCase(params[0]) || LUNA_BUY.equalsIgnoreCase(params[0]) || LUNA_Consume.equalsIgnoreCase(params[0])) {
			handleUpdate(player, params);
		}
	}
	...
	protected void showHelp(Player player) {
		PacketSendUtility.sendMessage(player, "//test pro|con|star| <int values>");
	}
}