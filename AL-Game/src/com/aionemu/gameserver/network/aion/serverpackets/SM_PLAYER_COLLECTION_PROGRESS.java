...
public class SM_PLAYER_COLLECTION_PROGRESS extends AionServerPacket {
    ...
    protected void writeImpl(AionConnection con) {
        writeH(this.player.getPlayerCollection().getPlayerCollectionEntry().size());
        for(PlayerCollectionEntry entry : this.player.getPlayerCollection().getPlayerCollectionEntry().values()) {
            writeD(entry.getId());
