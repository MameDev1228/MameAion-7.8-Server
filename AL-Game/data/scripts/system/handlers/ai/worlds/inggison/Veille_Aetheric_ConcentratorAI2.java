		...
		switch (getNpcId()) {
			...
			case 296907:
				if (player.getLevel() >= 80) {
					announceVeilleI();
					AI2Actions.targetCreature(Veille_Aetheric_ConcentratorAI2.this, getPosition().getWorldMapInstance().getNpc(840190)); //Asended Veille's Agent.
	...
	private void announceVeilleI() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				...
				PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_MSG_GODELITE_BUFF_FIRST_OBJECT_ON, 0);
			}
		});
	}
	...
	private void announceVeilleII() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				...
				PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_MSG_GODELITE_BUFF_SECOND_OBJECT_ON, 0);
			}
		});
	}
