...
public class StigmaService
{
    ...
    public static boolean notifyEquipAction(final Player player, Item resultItem, long slot) {
        ...
        if (resultItem.getItemTemplate().isStigma()) {
			...
			//All Stigma Skills +10
			if (player.getStigmaSet() >= 10) {
                addStigmaSetEnchant(player, 10);
            }
			//Somes Stigma Stone can give 2 Skills.
			if (itemId == 140001107) { //Exhausting Wave.
                player.getSkillList().addStigmaSkill(player, 539, 1 + resultItem.getEnchantLevel());
				player.getSkillList().addStigmaSkill(player, 749, 1 + resultItem.getEnchantLevel());
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player, player.getSkillList().getStigmaSkills()));
            } else if (itemId == 140001726) { //Exhausting Wave 7.x
                player.getSkillList().addStigmaSkill(player, 6027, 1 + resultItem.getEnchantLevel());
				player.getSkillList().addStigmaSkill(player, 6028, 1 + resultItem.getEnchantLevel());
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player, player.getSkillList().getStigmaSkills()));
            } else if (itemId == 140001218) { //Annihilation.
                player.getSkillList().addStigmaSkill(player, 1640, 1 + resultItem.getEnchantLevel());
				player.getSkillList().addStigmaSkill(player, 1883, 1 + resultItem.getEnchantLevel());
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player, player.getSkillList().getStigmaSkills()));
            } else if (itemId == 140001837) { //Annihilation 7.x
                player.getSkillList().addStigmaSkill(player, 6168, 1 + resultItem.getEnchantLevel());
				player.getSkillList().addStigmaSkill(player, 6169, 1 + resultItem.getEnchantLevel());
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player, player.getSkillList().getStigmaSkills()));
            }
        }
        return true;
    ...
    public static boolean notifyUnequipAction(final Player player, Item resultItem) {
        if (player.getEquipment().isSlotEquipped(ItemSlot.STIGMA_SPECIAL.getSlotIdMask()) && resultItem.getEquipmentSlot() != ItemSlot.STIGMA_SPECIAL.getSlotIdMask()) {
			...
			//Somes Stigma Stone can give 2 Skills.
			if (itemId == 140001107) { //Exhausting Wave.
                SkillLearnService.removeSkill(player, 539);
				SkillLearnService.removeSkill(player, 749);
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player, player.getSkillList().getStigmaSkills()));
            } else if (itemId == 140001726) { ////Exhausting Wave 7.x
                SkillLearnService.removeSkill(player, 6027);
				SkillLearnService.removeSkill(player, 6028);
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player, player.getSkillList().getStigmaSkills()));
            } else if (itemId == 140001218) { //Annihilation.
                SkillLearnService.removeSkill(player, 1640);
				SkillLearnService.removeSkill(player, 1883);
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player, player.getSkillList().getStigmaSkills()));
            } else if (itemId == 140001837) { //Annihilation 7.x
                SkillLearnService.removeSkill(player, 6168);
				SkillLearnService.removeSkill(player, 6169);
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player, player.getSkillList().getStigmaSkills()));
            }
        }
        return true;
        ...
        for (Item item : player.getEquipment().getEquippedItemsAllStigma()) {
            if (list.size() >= 6) {
                if (item.getEnchantLevel() == 6) {
                    ...
                    player.setStigmaSet(4);
                } else if (item.getEnchantLevel() == 10) {
                    player.setStigmaSet(5);
                } else if (item.getEnchantLevel() == 12) {
                    player.setStigmaSet(7);
                } else if (item.getEnchantLevel() == 14) {
                    player.setStigmaSet(9);
                } else if (item.getEnchantLevel() >= 15) {
                    player.setStigmaSet(10);
                } else {
                    player.setStigmaSet(0);
                }
            }
    ...
    private static int getPossibleStigmaCount(Player player) {
        ...
        if (player.getRace() == Race.ELYOS) {
            ...
            player.getQuestStateList().getQuestState(2900).getQuestVars().getQuestVars() == 98;
        }
        ...
        if (isCompleteQuest) {
            if (playerLevel < 30) {
                ...
                return 6;
            } else if (playerLevel < 55 && player.getStigmaSet() >= 5) {
                return 9;
            } else {
                return 9;
            }
    ...
    private static boolean isPossibleEquippedStigma(final Player player, Item item) {
            ...
            if (stigmaCount > 0) {
                if (stigmaCount == 1) {
                    ...
                    if (itemSlotToEquip == ItemSlot.STIGMA1.getSlotIdMask()
                        ...
                        || itemSlotToEquip == ItemSlot.STIGMA6.getSlotIdMask()
                        || itemSlotToEquip == ItemSlot.STIGMA_SPECIAL.getSlotIdMask()
						|| itemSlotToEquip == ItemSlot.STIGMA_SPECIAL_2.getSlotIdMask()) {
                        return true;
                    }
                } else if (stigmaCount == 9) {
                    if (itemSlotToEquip == ItemSlot.STIGMA1.getSlotIdMask()
                        || itemSlotToEquip == ItemSlot.STIGMA2.getSlotIdMask()
                        || itemSlotToEquip == ItemSlot.STIGMA3.getSlotIdMask()
                        || itemSlotToEquip == ItemSlot.STIGMA4.getSlotIdMask()
                        ...
                        || itemSlotToEquip == ItemSlot.STIGMA_SPECIAL.getSlotIdMask()
						|| itemSlotToEquip == ItemSlot.STIGMA_SPECIAL_2.getSlotIdMask()
						|| itemSlotToEquip == ItemSlot.STIGMA_SPECIAL_3.getSlotIdMask()) {
                        return true;
                    }
                }
            }
