...
@AIName("Esoterrace_WindStream_8")
public class Esoterrace_WindStream_8AI2 extends NpcAI2
{
	@Override
	...
	private void startWindStream(final Npc npc) {
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                Npc npc2 = (Npc) spawn(302850000, 703116, 392.0000f, 543.0000f, 318.0000f, (byte) 18, 0, 1);
				windStreamAnnounce(npc2, 0);
				WorldMapInstance instance = getPosition().getWorldMapInstance();
				deleteNpcs(instance.getNpcs(703115));
                spawn(703116, 392.0000f, 543.0000f, 318.0000f, (byte) 18);
                PacketSendUtility.broadcastPacket(npc2, new SM_WINDSTREAM_ANNOUNCE(1, 302850000, 455, 0));
                if (npc2 != null) {
    ...
    private void windStreamAnnounce(final Npc npc, final int state) {
        ...
        npc.getPosition().getWorld().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
                PacketSendUtility.sendPacket(player, new SM_WINDSTREAM_ANNOUNCE(1, 302850000, 455, state));
            }
        });
    }
