...
public class HarmonyArenaInstance extends GeneralInstanceHandler
{
	...
	private int getNpcBonus(int npcId) {
		switch (npcId) {
			case 207102:
			case 207116:
			case 654978: //Roaming Volcanic Petrahulk.
				return 400;
			case 207099:
				return 200;
			case 654979: //Heated Negotiator Grangvolkan.
				return 100;
			case 219328: //Plaza Wall.	
			case 654980: //Lurking Fangwing.
				return 50;
			default:
				return 0;
		}
	...
	protected void reward() {
		if (instanceReward.canRewarded()) {
			for (Player player : instance.getPlayersInside()) {
				...
				//<Abyss Points>
				AbyssPointsService.addAp(player, group.getBasicAP() + group.getRankingAP() + (int) (group.getScoreAP()));
				//<Glory Points>
				AbyssPointsService.addGp(player, group.getBasicGP() + group.getRankingGP() + (int) (group.getScoreGP()));
				int kinah = group.getBasicKinah() + group.getRankingKinah() + (int) (group.getScoreKinah());
				if (kinah != 0) {
