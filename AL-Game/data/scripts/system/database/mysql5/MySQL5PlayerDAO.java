...
public class MySQL5PlayerDAO extends PlayerDAO
{
	...
	public boolean isNameUsed(final String name) {
		PreparedStatement s = DB.prepareStatement("SELECT 1 FROM players WHERE name = ? LIMIT 1");
		try {
			s.setString(1, name);
	...
	public void storePlayer(final Player player) {
		...
		try {
			...
			PreparedStatement stmt = con.prepareStatement("UPDATE players SET name=?, exp=?, recoverexp=?, x=?, y=?, z=?, heading=?, world_id=?, gender=?, race=?, player_class=?, last_online=?, quest_expands=?, npc_expands=?, advenced_stigma_slot_size=?, warehouse_size=?, note=?, title_id=?, bonus_title_id=?, dp=?, soul_sickness=?, mailbox_letters=?, reposte_energy=?, mentor_flag_time=?, world_owner=?, join_legion_id=?, join_state=?, berdin_star=?, abyss_favor=?, world_play_time=?, luna_consume=?, muni_keys=?, luna_consume_count=?, wardrobe_slot=?, frenzy_points=?, frenzy_count=?, toc_floor=?, minion_energy=?, last_minion=?, golden_dice=?, sweep_reset=?, level=? WHERE id=?");
			...
			stmt.setInt(26, pcd.getJoinRequestLegionId());
            stmt.setString(27, pcd.getJoinRequestState().toString());
			stmt.setLong(28, pcd.getBerdinStar());
			stmt.setLong(29, pcd.getAbyssFavor());
			stmt.setInt(30, pcd.getWorldPlayTime());
			stmt.setInt(31, pcd.getLunaConsumePoint());
			stmt.setInt(32, pcd.getMuniKeys());
			stmt.setInt(33, pcd.getLunaConsumeCount());
			stmt.setInt(34, pcd.getWardrobeSlot());
			stmt.setInt(35, player.getUpgradeArcade().getFrenzyPoints());
			stmt.setInt(36, player.getUpgradeArcade().getFrenzyCount());
			stmt.setInt(37, pcd.getFloor());
			stmt.setInt(38, pcd.getMinionEnergy());
			stmt.setInt(39, pcd.getLastMinion());
			stmt.setInt(40, pcd.getGoldenDice());
			stmt.setInt(41, pcd.getResetBoard());
			stmt.setInt(42, pcd.getLevel());
	...
	public boolean saveNewPlayer(final PlayerCommonData pcd, final int accountId, final String accountName) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement preparedStatement = con.prepareStatement("INSERT INTO players(id, `name`, account_id, account_name, x, y, z, heading, world_id, gender, race, player_class, quest_expands, npc_expands, warehouse_size, bonus_title_id, wardrobe_slot, online, level) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 2, 0, 1)");
	...
	public PlayerCommonData loadPlayerCommonData(final int playerObjId) {
		...
		PreparedStatement stmt = DB.prepareStatement("SELECT * FROM players WHERE id = ?");
		try {
			...
			if (resultSet.next()) {
				...
				cd.setLevel(resultSet.getInt("level"));
	...
	public int getCharacterCountForRace(Race race) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con
				.prepareStatement("SELECT COUNT(DISTINCT(`account_name`)) AS `count` FROM `players` WHERE `race` = ? AND `level` >= ?");
			stmt.setString(1, race.name());
			stmt.setInt(2, GSConfig.RATIO_MIN_REQUIRED_LEVEL);
			ResultSet rs = stmt.executeQuery();
			rs.next();
