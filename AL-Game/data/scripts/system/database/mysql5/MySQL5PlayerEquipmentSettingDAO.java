...
public class MySQL5PlayerEquipmentSettingDAO extends PlayerEquipmentSettingDAO
{
    ...
    public void loadEquipmentSetting(Player player) {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                ringRight, necklace, shoulder, pants, powershardLeft, powershardRight, wings, waist, mOffHand, sOffHand, plume, bracelet, false);
            }
            rset.close();
            stmt.close();
        } catch (Exception e) {
    ...
    @Override
    public void deleteEquipmentSetting(Player player, int slotId) {
        Connection con = null;
        try {
            con = DatabaseFactory.getConnection();
