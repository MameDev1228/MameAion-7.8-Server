...
public class Contaminated_Underpath extends GeneralInstanceHandler
{
	private int rank;
	...
	public void onOpenDoor(Player player, int doorId) {
		if (doorId == 28) {
			...
			PacketSendUtility.sendSys3Message(player, "\uE005", "If you enter inside the <Contaminated Basin>, you are dead !!!");
			//The defense turret platform has appeared.
			//You can use Bright Aether to transform it for 15 seconds.
			sendMsgByRace(1403696, Race.PC_ALL, 3000);
		}
	}
	...
	public void onEnterInstance(final Player player) {
		...
		contaminatedUnderpath(player);
		final int lunaHeal = spawnRace == Race.ASMODIANS ? 703477 : 703477;
		spawn(lunaHeal, 236.77367f, 212.99107f, 160.28148f, (byte) 60);
	}
	
	public static final void contaminatedUnderpath(final Player player) {
		player.getSkillList().addSkill(player, 411, 1);
		player.getSkillList().addSkill(player, 412, 1);
		player.getSkillList().addSkill(player, 413, 1);
		player.getSkillList().addSkill(player, 414, 1);
		player.getSkillList().addSkill(player, 415, 1);
	}
	...
	private void removeEffects(Player player) {
		...
		///////////////////////////////////////////
		SkillLearnService.removeSkill(player, 411);
		SkillLearnService.removeSkill(player, 412);
		SkillLearnService.removeSkill(player, 413);
		SkillLearnService.removeSkill(player, 414);
		SkillLearnService.removeSkill(player, 415);
	}
