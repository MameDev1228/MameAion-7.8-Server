...
public class QUEST_Q70601 extends QuestHandler
{
	private final static int questId = 70601;
	private final static int[] npcs = {806986, 806987, 806988, 806990, 703704, 731930, 807000, 836014, 836015};
	...
	private final static int[] LDF8_B3_M_Lega_D_Fi_80_Ae = {657415};
	////////////////////////////////////////////////////////////////
	...
	public void register() {
		...
		for (int npc: npcs) {
		    ...
		    qe.registerQuestNpc(mob).addOnKillEvent(questId);
		} for (int mob: LDF8_B3_M_Lega_D_Fi_80_Ae) {
		    qe.registerQuestNpc(mob).addOnKillEvent(questId);
    ...
    public boolean onLvlUpEvent(QuestEnv env) {
        return defaultOnLvlUpEvent(env, 70600, true);
    }
    ...
    public boolean onDialogEvent(final QuestEnv env) {
		...
		if (qs == null || qs.getStatus() == QuestStatus.START) {
			if (targetId == 806986) {
				...
				switch (env.getDialog()) {
					case START_DIALOG: {
						...
						giveQuestItem(env, 182216762, 1);
						changeQuestStep(env, 3, 4, false);
						PacketSendUtility.sendSys3Message(player, "\uE005", "Save the wounded Asmodians!!!");
						return closeDialogWindow(env);
            ...
            } if (targetId == 836014 || targetId == 836015) {
                switch (env.getDialog()) {
                    case USE_OBJECT: {
						if (var == 4) {
							if (var1 >= 0 && var1 < 4) {
								...
								QuestService.addNewSpawn(800060000, 1, 836013, npc.getX(), npc.getY(), npc.getZ(), (byte) 0);
							} else if (var1 == 4) {
								qs.setQuestVar(5);
								updateQuestStatus(env);
								removeQuestItem(env, 182216762, 1);
								return closeDialogWindow(env);
							}
						}
					}
		...
		if (qs != null && qs.getStatus() == QuestStatus.START) {
			...
			if (var == 5) {
				...
				switch (targetId) {
					...
					case 657415:
						if (var3 < 0) {
							return defaultOnKillEvent(env, LDF8_B3_M_Lega_D_Fi_80_Ae, 0, 0, 3);
						} else if (var3 == 0) {
							if (var1 == 5 && var2 == 5) {
								qs.setQuestVar(6);
								updateQuestStatus(env);
								...
								return defaultOnKillEvent(env, LDF8_B3_M_Lega_D_Fi_80_Ae, 0, 1, 3);
							}
						}
					break;
				}
