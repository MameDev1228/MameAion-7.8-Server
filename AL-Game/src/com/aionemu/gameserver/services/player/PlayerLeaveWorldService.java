...
public class PlayerLeaveWorldService
{
	...
	public static final void startLeaveWorld(Player player) {
		...
		player.onLoggedOut();
		MameFfaService.onLogout(player);
		...
		if (EventsConfig.ENABLE_BOOST_EVENT) {
			BoostEventService.getInstance().onLogout(player);
		}
		...
		MonsterCoreService.getInstance().onLogout(player);
		...
		StigmaUpgradeLinkedService.onLogOut(player);
