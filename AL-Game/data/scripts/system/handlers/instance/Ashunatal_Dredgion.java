...
public class Ashunatal_Dredgion extends GeneralInstanceHandler
{
	...
	private final FastList<Future<?>> ashunatalTask = FastList.newInstance();
	...
	public void onDropRegistered(Npc npc) {
		...
		switch (npcId) {
			case 654814: //Frigate Commander Ashunatal.
				for (Player player: instance.getPlayersInside()) {
					if (player.isOnline()) {
						dropItems.add(DropRegistrationService.getInstance().regDropItem(index++, player.getObjectId(), npcId, 166401000, 250)); //Manastone Fastener.
	...
	private void onDieSurkan(Npc npc, Player mostPlayerDamage, int score) {
		...
		for (Player player: instance.getPlayersInside()) {
			...
			"\uE050", "<Frigate Commander Ashunatal> is now in the Captain's Cabin");
			spawn(654814, 485.0000f, 812.0000f, 416.0000f, (byte) 31); //Frigate Commander Ashunatal.
        }
	...
	protected void startInstanceTask() {
		...
		ashunatalTask.add(ThreadPoolManager.getInstance().schedule(new Runnable() {
			@Override
			public void run() {
				openFirstDoors();
				...
				switch (Rnd.get(1, 2)) {
					case 1:
					    spawn(654805, 415.0000f, 282.0000f, 409.0000f, (byte) 118); //Supervisor Gayaba.
					break;
					case 2:
					    spawn(654805, 556.0000f, 279.0000f, 409.0000f, (byte) 33); //Supervisor Gayaba.
					break;
				} switch (Rnd.get(1, 2)) {
					case 1:
						spawn(654813, 485.0000f, 877.0000f, 405.0000f, (byte) 90); //Vice Captain Anggiras.
					break;
					case 2:
					    spawn(654822, 485.0000f, 877.0000f, 405.0000f, (byte) 90); //Auditor Agwe.
					break;
				}
			}
		}, 60000));
		...
		*/
		ashunatalTask.add(ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				spawn(801989, 415.0000f, 173.0000f, 432.0000f, (byte) 0, 34); //Port Side Teleporter.
				spawn(801990, 554.0000f, 173.0000f, 432.0000f, (byte) 0, 9); //Starboard Side Teleporter.
			}
		}, 600000));
		/**
		* Large Bagitara:
		...
		*/
		ashunatalTask.add(ThreadPoolManager.getInstance().schedule(new Runnable() {
			@Override
			public void run() {
				spawn(654820, 485.0000f, 313.0000f, 403.0000f, (byte) 36); //Large Bagitara.
			}
		}, 900000));
		ashunatalTask.add(ThreadPoolManager.getInstance().schedule(new Runnable() {
			@Override
		...
		switch (npc.getObjectTemplate().getTemplateId()) {
			...
			*/
			case 654821: //Lieutenant Renuka.
				...
				if (race.equals(Race.ELYOS)) {
				   ...
				   spawn(801993, 473.0000f, 761.0000f, 388.0000f, (byte) 0, 33); //Elyos Captain's Cabin Teleporter.
				} else if (race.equals(Race.ASMODIANS)) {
				   //A teleportation device to the Captain's Cabin is now available at the end of the Atrium.
				   sendMsgByRace(1401901, Race.ASMODIANS, 0);
				   spawn(801994, 473.0000f, 761.0000f, 388.0000f, (byte) 0, 186); //Asmodian Captain's Cabin Teleporter.
				}
            break;
		   /**
			...
			*/
			case 802001: //Portside Teleporter Generator.
				...
				spawn(801991, 397.0000f, 184.0000f, 432.0000f, (byte) 0, 42); //Port Supply Room Teleporter.
            break;
			case 802002: //Starboard Teleporter Generator.
				...
				spawn(801992, 572.0000f, 185.0000f, 432.0000f, (byte) 0, 10); //Starboard Supply Room Teleporter.
            break;
		   /**
			...
			*/
			...
			case 802006: //Starboard Bulkhead.
				bulkhead++;
				if (bulkhead == 2) {
					spawn(654815, 456.0000f, 319.0000f, 402.0000f, (byte) 28); //Menehune The Sprightly.
				}
				despawnNpc(npc);
			break;
			case 654806: //Chief Navigator Yatri.
			case 654807: //Assistant Navigator Magoyart.
			case 654808: //Arebusqeirs Sub Captain Darma.
			case 654809: //Arebusqeirs Captain Magus.
			case 654810: //Airborne Captain Lagabrinta.
			case 654811: //Airborne Captain Filitu.
			case 654812: //Technician Mursillis.
			case 654817: //Supply Captain Varuk.
			case 654818: //Supply Vice Captain Aketaton.
			    secretCache++;
				if (secretCache == 5) {
				    //A Dredgion Treasure Chest is now available in the Drop Zone!
					sendMsgByRace(1401902, Race.PC_ALL, 0);
					spawn(701455, 482.0000f, 496.0000f, 397.0000f, (byte) 92); //Dredgion Opportunity Bundle.
				}
			break;
			case 654813: //Vice Captain Anggiras.
			case 654822: //Auditor Agwe.
			    score += 500;
            break;
			case 654805: //Supervisor Gayaba.
			case 654819: //Gatekeeper Menes.
			case 654820: //Large Bagitara.
				score += 1000;
			break;
			case 654815: //Menehune The Sprightly.
			    score += 1000;
				AbyssPointsService.addGp(mostPlayerDamage, 300);
			break;
			case 654816: //Raima The Cruel.
				AbyssPointsService.addGp(mostPlayerDamage, 300);
			break;
			case 654814: //Frigate Commander Ashunatal.
				score += 1000;
	...
	public void onEnterInstance(final Player player) {
		...
		if (!containPlayer(player.getObjectId())) {
			...
			ClassChangeService.onUpdateQuest16020(player);
		} else {
			ClassChangeService.onUpdateQuest26020(player);
		}
		sendPacket();
	}
	...
	private void stopInstanceTask() {
        for (FastList.Node<Future<?>> n = ashunatalTask.head(), end = ashunatalTask.tail(); (n = n.getNext()) != end; ) {
            if (n.getValue() != null) {
