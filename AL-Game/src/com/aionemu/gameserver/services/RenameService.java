...
public class RenameService
{
    ...
    if (!NameRestrictionService.isValidName(newName)) {
			...
			PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1400155));
			return false;
		} if ((player.getInventory().getItemByObjId(item).getItemId() != 169670000 && //Name Change Ticket.
		    player.getInventory().getItemByObjId(item).getItemId() != 169670001 && //[Event] Name Change Ticket.
			player.getInventory().getItemByObjId(item).getItemId() != 169670002 && //Name Change Ticket.
			player.getInventory().getItemByObjId(item).getItemId() != 169670010) || //[Event] Character Name Change Ticket.
		...
		while (onlinePlayers.hasNext()) {
			...
			if (p != null && p.getClientConnection() != null) {
				...
				PacketSendUtility.sendPacket(p, new SM_PLAYER_INFO(player, false));
			}
		}
		return true;
	}
