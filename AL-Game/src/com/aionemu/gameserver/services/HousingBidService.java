...
public class HousingBidService extends AbstractCronTask
{
    ...
    protected void executeTask() {
        ...
        for (Entry<Integer, HouseBidEntry> playerBid : playerBids.entrySet()) {
            ...
            if (houseBid == null) {
                log.warn("Skipping stale house bid for player " + playerId + ": missing bid entry index " + playerBid.getValue().getEntryIndex());
                continue;
            }
            House house = HousingService.getInstance().getHouseByAddress(houseBid.getAddress());
            if (house == null) {
                log.warn("Skipping stale house bid for player " + playerId + ": missing house address " + houseBid.getAddress());
                continue;
            }
            ...
            House house = HousingService.getInstance().getHouseByAddress(houseBid.getAddress());
            if (house == null) {
                log.warn("Skipping stale house auction entry: missing house address " + houseBid.getAddress());
                continue;
            }
            ...
            if (wonHouse == null) {
                log.warn("Skipping winning house bid for player " + winData.getValue() + ": missing house address " + winData.getKey().getAddress());
                continue;
            }
            PlayerCommonData winnerPcd = getPlayerData(winData.getValue());
            if (winnerPcd == null) {
                log.warn("Missing Player with ID:" + winData.getValue() + " for Housebid on address:" + winData.getKey().getAddress());
                continue;
            }
            AuctionResult result = completeHouseSell(winnerPcd, wonHouse);
        }
        ...
        if (LoggingConfig.LOG_HOUSE_AUCTION) {
            ...
            if (soldHouse == null) {
                log.warn("Skipping successful house auction: missing house address " + sellData.getKey().getAddress());
                continue;
            }
            PlayerCommonData buyerPcd = getPlayerData(sellData.getValue());
            PlayerCommonData sellerPcd = getPlayerData(soldHouse.getOwnerId());
            if (buyerPcd == null || sellerPcd == null) {
                log.warn("Skipping successful house auction for address " + sellData.getKey().getAddress() + ": buyer or seller data is missing");
                continue;
            }
            ...
            if (bidHouse == null) {
                log.warn("Skipping failed house auction: missing house address " + bidEntry.getAddress());
                continue;
            }
            if (sellerPcd == null) {
                log.warn("Skipping failed house auction for address " + bidEntry.getAddress() + ": seller player data is missing");
                continue;
            }
            if (sellerPcd.isOnline()) {
        ...
        if (LoggingConfig.LOG_HOUSE_AUCTION) {
            ...
            House house = HousingService.getInstance().getHouseByAddress(houseBid.getAddress());
            if (house == null) {
                log.warn("Skipping auction cleanup for missing house address " + houseBid.getAddress());
                continue;
            }
