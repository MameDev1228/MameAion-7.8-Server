...
public class IllumielBattlefieldReward extends InstanceReward<IllumielBattlefieldPlayerReward>
{
    ...
    public IllumielBattlefieldReward(Integer mapId, int instanceId, WorldMapInstance instance) {
	...
	public int AbyssReward(boolean isWin, boolean isTankKilled) {
    	int TankKilled = 50000;
    	int Win = 360000;
    	int Loss = 25000;
    	if (isTankKilled) {
    		return isWin ? (Win + TankKilled) : (Loss + TankKilled);
    	} else {
    		return isWin ? Win : Loss;
    	}
    }
	public int GloryReward(boolean isWin, boolean isTankKilled) {
    	int TankKilled = 50;
    	int Win = 200;
    	int Loss = 30;
    	if (isTankKilled) {
	...
	public int ExpReward(boolean isWin, boolean isTankKilled) {
    	int TankKilled = 20000;
    	int Win = 10000;
    	int Loss = 5000;
    	if (isTankKilled) {
    		return isWin ? (Win + TankKilled) : (Loss + TankKilled);
    ...
    public List<IllumielBattlefieldPlayerReward> sortPoints() {
    ...
    private void setStartPositions() {
        Point3D a = new Point3D(263.0000f, 166.0000f, 79.0000f);
        Point3D e = new Point3D(266.0000f, 351.0000f, 79.0000f);
    ...
    public void regPlayerReward(Player player) {
        if (!containPlayer(player.getObjectId())) {
            addPlayerReward(new IllumielBattlefieldPlayerReward(player.getObjectId(), bonusTime, buffId, player.getRace()));
        }
    }
	
    @Override
    public void addPlayerReward(IllumielBattlefieldPlayerReward reward) {
        super.addPlayerReward(reward);
    }
	
    @Override
    public IllumielBattlefieldPlayerReward getPlayerReward(Integer object) {
        return (IllumielBattlefieldPlayerReward) super.getPlayerReward(object);
    }
