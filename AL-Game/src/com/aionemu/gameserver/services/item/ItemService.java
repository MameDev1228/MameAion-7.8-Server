...
public class ItemService {
	...
	private static long addNonStackableItem(Player player, ItemTemplate itemTemplate, long count, Item sourceItem, ItemUpdatePredicate predicate, int enchantlevel, boolean augment) {
		...
		while (!inventory.isFull(itemTemplate.getExtraInventoryId()) && count > 0) {
			...
			if (newItem.getExpireTime() != 0) {
				...
				newItem.setOptionalSocket(-1);
				newItem.setBonusNumber(-1);
				newItem.setRndBonus(new FastMap<Integer, ItemRndBonus>());
			} if (enchantlevel > 0) {
				enchant(player, enchantlevel, newItem);
			} if (augment) {
				chargeItem(player, newItem, 2);
			} if (itemTemplate.getEnhanceTable() != 0) {
				EnchantService.addSkillEnhance(player, newItem);
			} if (itemTemplate.getVisionSkill() != 0) {
				newItem.setAmplificationSkill(itemTemplate.getVisionSkill());
			}
	...
	private static void copyItemInfo(Item sourceItem, Item newItem) {
		...
		} if (sourceItem.getAuthorizeLevel() > 0) {
			newItem.setAuthorizeLevel(sourceItem.getAuthorizeLevel());
		} if (sourceItem.getOptionalSocket() != 0) {
			newItem.setOptionalSocket(sourceItem.getOptionalSocket());
		} if (sourceItem.isSoulBound()) {
			newItem.setSoulBound(true);
		}
		newItem.setBonusNumber(sourceItem.getBonusNumber());
		newItem.setRndBonus(sourceItem.getRndBonus());
	...
	public static void makeUpgradeItem(Item sourceItem, Item newItem) {
		...
		} if (sourceItem.getEnchantPvPvELevel() > 0) {
			newItem.setEnchantPvPvELevel(sourceItem.getEnchantPvPvELevel());
		} if (sourceItem.isSoulBound()) {
			newItem.setSoulBound(true);
		} if (newItem.getItemTemplate().getRandomBonusId() != 0) {
			newItem.setOptionalSocket(-1);
		}
	...
	private static void enchant(Player player, int enchant, Item item) {
		if (isUpgradable(item)) {
			if (item.getEnchantPvPvELevel() == enchant) {
				return;
			} if (enchant > 20) {
				enchant = 20;
			} if (enchant < 0) {
			...
			item.setEnchantPvPvELevel(enchant);
			if (item.isEquipped()) {
				player.getGameStats().updateStatsVisually();
			}
			ItemPacketService.updateItemAfterInfoChange(player, item);
