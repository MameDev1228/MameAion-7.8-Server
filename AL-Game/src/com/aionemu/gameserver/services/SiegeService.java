...
public class SiegeService
{
	...
	public void broadcastUpdate(SiegeLocation loc) {
		...
		broadcast(new SM_SIEGE_LOCATION_INFO(loc), new SM_INFLUENCE_RATIO(), new SM_STRONGHOLDS(loc));
	}
	
	public void broadcast(final AionServerPacket pkt1, final AionServerPacket pkt2, final AionServerPacket pkt3) {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			public void visit(Player player) {
				if (pkt1 != null) {
					...
					PacketSendUtility.sendPacket(player, pkt3);
				}
			}
		});
	}
	...
	public void onPlayerLogin(Player player) {
		if (SiegeConfig.SIEGE_ENABLED) {
			...
			for(SiegeLocation loc : getSiegeLocations().values()) {
				PacketSendUtility.sendPacket(player, new SM_STRONGHOLDS(loc));
			}
		}
	}
