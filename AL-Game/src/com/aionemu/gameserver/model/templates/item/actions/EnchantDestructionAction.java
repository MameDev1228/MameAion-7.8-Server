...
public class EnchantDestructionAction extends AbstractItemAction
{
    @Override
    public boolean canAct(Player player, Item parentItem, Item targetItem) {
        if (parentItem == null || targetItem == null) {
            ...
            return false;
        } if (parentItem.getItemTemplate().isEnchantmentDestructionStone() && player.getInventory().getKinah() < EnchantService.EnchantKinah(targetItem)) {
    ...
    public void act(final Player player, final Item parentItem, final Item targetItem) {
        ...
        player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                EnchantService.enchantDestructionItemAct(player, parentItem, targetItem, targetItem.getEnchantPvPvELevel(), isSuccess);
                ...
                if (itemTemplate.isEnchantmentDestructionStone()) {
                    Iterator<Player> iter = World.getInstance().getPlayersIterator();
                    while (iter.hasNext()) {
                        Player player2 = iter.next();
                        if (targetItem.getEnchantLevel() >= 1 && targetItem.getEnchantLevel() <= 20 && isSuccess) {
                            if (player2.getRace() == player.getRace()) {
                                //%0 has succeeded in enchanting %1 to level %2.
                                PacketSendUtility.sendPacket(player2, SM_SYSTEM_MESSAGE.STR_MSG_ENCHANT_ITEM_SUCCEEDED_DESTRUCTION(player.getName(), targetItem.getItemTemplate().getNameId(), targetItem.getEnchantLevel()));
                            }
                        }
                    }
                }
    ...
    private boolean isSuccess(final Player player, final Item parentItem, final Item targetItem, final int targetWeapon) {
        if (parentItem.getItemTemplate() != null) {
            return EnchantService.enchantDesctructionItem(player, parentItem, targetItem);
        }
        return false;
    }
}