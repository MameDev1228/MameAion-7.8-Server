...
public class RideAction extends AbstractItemAction
{
	...
	public boolean canAct(Player player, Item parentItem, Item targetItem) {
		if (parentItem == null) {
			...
			//You cannot get on your mount in your current form. You cannot use the item.
			PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_MSG_CANNOT_RIDE_POLYMORPH_STATUS);
			return false;
		} if (CustomConfig.ENABLE_RIDE_RESTRICTION) {
			for (ZoneInstance zone: player.getPosition().getMapRegion().getZones(player)) {
				if (!zone.canRide()) {
					//You cannot ride here. You are forced to dismount.
	...
	public void act(final Player player, final Item parentItem, Item targetItem) {
		...
		player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				//Add Ride Buff.
				SkillEngine.getInstance().applyEffectDirectly(getRideInfo().getBuffId(), player, player, 0);
