...
public class AttackUtil
{
	 ...
	 * @param accMod
	 */
	public static void calculateSkillResult(Effect effect, int skillDamage, ActionModifier modifier, Func func, int randomDamage, int accMod, float critProbMod2, int critAddDmg, boolean cannotMiss, boolean shared, boolean ignoreShield, boolean isMainHand) {
		...
		int damageBeforeSkill = damage;
		int appliedSkillDamage = skillDamage;
		if (func != null) {
			switch (func) {
				case ADD:
					if (physicalSkillFormulaProbe) {
						appliedSkillDamage = MameClientCompatDebug.applyPhysicalSkillDamageMode(effector, effected, skillDamage);
					}
					damage += appliedSkillDamage;
				break;
				case PERCENT:
					appliedSkillDamage = Math.round(baseAttack * skillDamage / 100f);
					damage += appliedSkillDamage;
				break;
			}
		}
		if (physicalSkillFormulaProbe) {
			MameClientCompatDebug.logPhysicalSkillFormula(effect, skillDamage, damageBeforeSkill, appliedSkillDamage, damage, func == null ? "null" : func.name());
		}
		if (modifier != null) {
			int bonus = modifier.analyze(effect);
		...
		if (effector.getAttackType() == ItemAttackType.PHYSICAL) {
			status = calculatePhysicalStatus(effector, effected, true, accMod, critProbMod2, true, cannotMiss);
		} else {
	 ...
	 * @return
	 */
	public static int calculateMagicalOverTimeSkillResult(Effect effect, int skillDamage, SkillElement element, int position, boolean useMagicBoost, float critProbMod2, int critAddDmg) {
		Creature effector = effect.getEffector();
		Creature effected = effect.getEffected();
	...
	public static void calculateMagicalSkillResult(Effect effect, int skillDamage, ActionModifier modifier, SkillElement element, boolean useMagicBoost, boolean useKnowledge, boolean noReduce, Func func, float critProbMod2, int critAddDmg, boolean shared, boolean ignoreShield) {
		Creature effector = effect.getEffector();
		Creature effected = effect.getEffected();
	...
	public static AttackStatus calculatePhysicalStatus(Creature attacker, Creature attacked, boolean isMainHand, int accMod, float critProbMod2, boolean isSkill, boolean cannotMiss) {
		AttackStatus status = AttackStatus.NORMALHIT;
		if (!isMainHand)
	  ...
	  StatFunctions.calculatePhysicalBlockRate(attacker, attacked);
		...
		if (StatFunctions.calculatePhysicalCriticalRate(attacker, attacked, isMainHand, critProbMod2, isSkill)) {
			switch (status) {
			case BLOCK:
				if (isMainHand)
	 ...
	 */
	public static AttackStatus calculateMagicalStatus(Creature attacker, Creature attacked, float critProbMod2, boolean isSkill) {
		if (!isSkill) {
			if (Rnd.get(0, 1000) < StatFunctions.calculateMagicalResistRate(attacker, attacked, 0))
				return AttackStatus.RESIST;
		}

		if (StatFunctions.calculateMagicalCriticalRate(attacker, attacked, critProbMod2)) {
	...
	public static boolean isSkillEffect(int skillId) {
		switch (skillId) {
			...
			********************/
			...
			case 4728: //Aetherflame.
			case 4790: //Sonic Gust.
			case 5101: //Rupture Arrow Burst.
			case 5102: //Rupture Arrow Onslaught.
			case 5146: //Fiery Requiem.
			case 5260: //Shifting Magma.
			case 5598: //Chroma Burst.
			case 5707:
			case 5750:
			case 5751:
			case 5785:
			...
			********************/
			...
			case 5083: //Crushing Onslaught.
			case 5084: //Crushing Assault.
			case 5137: //Idium Slam.
			case 5140: //Idium Barrage.
			case 5252: //Relentless Cleave.
			case 5278: //Ruthless Ambush.
			case 5282: //Sweeping Crash.
			case 5720:
			case 5721:
			case 5725:
			case 5747:
			...
			***********************/
			...
			case 5559: //Acrylic Eruption I
			case 5560: //Acrylic Eruption II
			case 5561: //Acrylic Eruption III.
			case 5562: //Acrylic Eruption IV.
			case 5563: //Acrylic Eruption V.
			case 5564: //Acrylic Eruption VI.
			case 6204:
			case 6290:
			case 13350:
		   /*******************
			...
			*******************/
			...
			case 429: //Aether Leash.
			case 440: //Aether Leash.
			case 451: //Twisting Chains.
			case 458: //Area Chains.
			case 2967: //Illusion Chains.
			case 4721: //Entangling Chains.
			...
			case 4736: //Sweeping Hook.
			case 5185: //Divine Grasp I.
			case 5186: //Divine Grasp II.
			case 5187: //Divine Grasp III.
			case 9168:
			case 9395:
			case 9440:
			case 10483:
	...
	public static void applyEffectOnCritical(Player attacker, Creature attacked, int returnSkill) {
		...
		for (Effect ef: attacked.getEffectController().getAbnormalEffects()) {
			//Remove Shock & Archdeava Remove Shock.
			if (ef.getSkillId() == 283 || ef.getSkillId() == 4830 || ef.getSkillId() == 4840 ||
				ef.getSkillId() == 4841 || ef.getSkillId() == 4842 || ef.getSkillId() == 4863 ||
				ef.getSkillId() == 4864 || ef.getSkillId() == 4865) {
				return;
			}
		}
