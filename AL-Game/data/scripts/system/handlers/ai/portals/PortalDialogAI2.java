...
public class PortalDialogAI2 extends PortalAI2
{
	...
	protected void handleSpawned() {
		super.handleSpawned();
		switch (getNpcId()) {
			case 730256: //Silentera Westgate.
			    startMsgTask();
			break;
        }
	}
	...
	private void startMsgTask() {
		msgTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new Runnable() {
			@Override
			public void run() {
				silenteraOpen();
			}
	...
	private void silenteraOpen() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				...
				PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_FIELDABYSS_DARKUNDERPASS_SPAWN, 10000);
			}
		});
	}
