...
@AIName("DF4_OP1_Boss_Tegrak")
public class Ascended_Marchutan_AgentAI2 extends AggressiveNpcAI2
{
	...
	private void announceAgentUnderAttack() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				...
				PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_FIELDABYSS_DARKBOSS_ATTACKED, 0);
			}
		});
	}
	...
	private void announceGodEliteStart4() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				...
				PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_FIELDABYSS_DARKBOSS_SPAWN, 0);
				...
				PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_MSG_DF4_GODELITE_START_4, 10000);
			}
		});
	}
	...
	private void announceDespawn() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				...
				PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_FIELDABYSS_DARKBOSS_DESPAWN, 0);
			}
		});
	}
	...
	protected void handleSpawned() {
		...
		//Mastarius's Flag.
		spawn(832831, getOwner().getX(), getOwner().getY(), getOwner().getZ(), (byte) getOwner().getHeading());
	}
    ...
    protected void handleDied() {
		...
		deleteNpcs(instance.getNpcs(832831)); //Mastarius's Flag.
    }
