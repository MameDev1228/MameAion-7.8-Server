...
import com.aionemu.gameserver.questEngine.handlers.models.Reward;
...
public final class QuestService
{
	...
	public static boolean finishQuest(QuestEnv env, int reward) {
		...
		if (template.getCategory() == QuestCategory.EPISODE &&
		    template.getCategory() == QuestCategory.GUIDE &&
			template.getCategory() == QuestCategory.MISSION && qs.getCompleteCount() != 0) {
			return false;
		}
	...
	public static void bountyReward(QuestEnv env, int index) {
		Player player = env.getPlayer();
		QuestTemplate template = DataManager.QUEST_DATA.getQuestById(env.getQuestId());
		Rewards reward = template.getRewards().get(0);
		Rewards rewards = new Rewards();
		QuestItems classRewardItem = null;
		int id = env.getQuestId();
		int selRewIndex = index;
		QuestState qs = player.getQuestStateList().getQuestState(id);
		Rewards extendedRewards = new Rewards();
		if (qs == null || qs.getStatus() != QuestStatus.REWARD) {
			return;
		...
		if (!template.getExtendedRewards().isEmpty()) {
			if (qs.getCompleteCount() == template.getMaxRepeatCount() - 1) {
				questItems.addAll(getRewardItems(env, template, true, 0));
				extendedRewards = template.getExtendedRewards().get(0);
			}
		} if (!template.getRewards().isEmpty() || !template.getBonus().isEmpty()) {
			questItems.addAll(getRewardItems(env, template, false, 0));
			rewards = template.getRewards().get(0);
		} if (ItemService.addQuestItems(player, questItems)) {
			giveReward(env, rewards);
			giveReward(env, extendedRewards);
			if (template.getCategory() == QuestCategory.CHALLENGE_TASK) {
			...
			setFinishingState(env, template, 0);
		} if (player.getInventory().isFull()) {
			PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_WAREHOUSE_FULL_INVENTORY);
			return;
		} if (template.isUseSingleClassReward()) {
			...
			switch (playerClass) {
				case ASSASSIN: {
					...
					classRewardItem = getQuestItemsbyClass(id, template.getVandalSelectableReward(), selRewIndex);
					break;
				}
				default:
				break;
			}
		} if (!template.getRewards().get(0).getSelectableRewardItem().isEmpty()) {
			QuestItems selectedReward = reward.getSelectableRewardItem().get(index);
			ItemService.addItem(player, selectedReward.getItemId(), selectedReward.getCount());
		} if (classRewardItem != null) {
			ItemService.addItem(player, classRewardItem.getItemId(), classRewardItem.getCount());
		}
		player.getController().updateZone();
		player.getController().updateNearbyQuests();
	}
	...
	private static void giveReward(QuestEnv env, Rewards rewards) {
		...
		if (rewards.getFameExp() != null) {
			PlayerFameService.getInstance().addFameExp(player, rewards.getFameExp());
		}
		//Minion Energy.
