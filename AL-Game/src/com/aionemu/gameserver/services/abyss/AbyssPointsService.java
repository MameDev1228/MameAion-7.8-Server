...
public class AbyssPointsService
{
	...
	public static void addAp(Player player, int value) {
		...
		} if (player.getCommonData().isReadyForAbyssFavor() && player.getCommonData().getAbyssFavor() > 0) {
			abyssFavorBoost = (long) ((value / 100f) * 50);
			player.getCommonData().deleteAbyssFavor(abyssFavorBoost);
			PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_GET_EXP_DESC(new DescriptionId(2808053), abyssFavorBoost));
		}
		value = (int) (value + abyssFavorBoost);
		setAp(player, value);
	...
	public static void setGp(Player player, int value) {
		...
		AbyssRank rank = player.getAbyssRank();
		rank.addGp(value);
		SeasonRankingService.getInstance().addGloryPlayerScore(player, value);
		PacketSendUtility.sendPacket(player, new SM_ABYSS_RANK(player.getAbyssRank()));
	}
