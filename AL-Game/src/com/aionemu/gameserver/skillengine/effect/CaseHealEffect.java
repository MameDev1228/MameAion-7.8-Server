...
public class CaseHealEffect extends AbstractHealEffect
{
	...
	private void calculateHeal(final Effect effect) {
		...
		if (condValue == 80) {
			//Celestial Weapon 7.x
			if (effect.getSkillId() == 13409) {
				SkillEngine.getInstance().applyEffectDirectly(9567, effector, effected, 0);
			} if (effect.getSkillId() == 13410) {
				SkillEngine.getInstance().applyEffectDirectly(9568, effector, effected, 0);
			} if (effect.getSkillId() == 13411) {
				SkillEngine.getInstance().applyEffectDirectly(9569, effector, effected, 0);
			}
