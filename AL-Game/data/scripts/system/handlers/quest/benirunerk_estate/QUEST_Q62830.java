...
public class QUEST_Q62830 extends QuestHandler
{
	private final static int questId = 62830;
	...
	public void register() {
		...
		qe.registerQuestNpc(806977).addOnQuestStart(questId);
		...
		qe.registerOnEnterZone(ZoneName.get("IDF8_HOUSE_ITEMUSEAREA_Q62830A"), questId);
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE) {
            ...
            if (targetId == 838346) {
                if (env.getDialog() == QuestDialog.USE_OBJECT) {
					...
					removeQuestItem(env, 182216945, 1);
    ...
    public HandlerResult onItemUseEvent(QuestEnv env, final Item item) {
		...
		if (id == 182216945) {
            if (var == 4 && player.isInsideZone(ZoneName.get("IDF8_HOUSE_ITEMUSEAREA_Q62830A"))) {
				qs.setQuestVar(5);
				updateQuestStatus(env);
				//Let's go to the cliff to wait for Kyle.
				PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_CHAT_Q62830_Gossip_01, 0);
