...
public class OutpostService
{
	...
	public void capture(int id, Race race) {
        ...
        stop(id);
        World.getInstance().getWorldMap(getOutpostLocation(id).getWorldId()).getMainWorldMapInstance().doOnAllPlayers(new Visitor<Player>() {
            @Override
            public void visit(Player player) {
                onEnterOutpostWorld(player);
            }
        });
