...
public class IdgelDomeService
{
	...
	public void initIdgelDome() {
		if (AutoGroupConfig.IDGEL_ENABLED) {
			//Idgel Dome TUE,FRI,SUN "12PM-1PM"
			...
			//Idgel Dome TUE,FRI,SUN "8PM-1AM"
			CronService.getInstance().schedule(new Runnable() {
				@Override
				public void run() {
					startIdgelRegistration2();
				}
			}, AutoGroupConfig.IDGEL_SCHEDULE_NIGHT);
		}
	}
	...
	private void startUregisterIdgelTask2() {
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                registerAvailable = false;
        ...
        }, AutoGroupConfig.IDGEL_TIMER_2 * 60 * 1000);
    }
	
	private void startIdgelRegistration2() {
        this.registerAvailable = true;
        startUregisterIdgelTask2();
        Iterator<Player> iter = World.getInstance().getPlayersIterator();
        while (iter.hasNext()) {
            Player player = iter.next();
            if (player.getLevel() > minLevel && player.getLevel() < capLevel) {
