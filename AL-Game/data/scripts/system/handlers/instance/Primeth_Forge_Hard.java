...
@InstanceID(302630000)
public class Primeth_Forge_Hard extends GeneralInstanceHandler
{
    ...
    public void onDropRegistered(Npc npc) {
		...
		switch (npcId) {
			...
			break;
			case 839628: //Berserking Yastikan-Berserking Varakan Cubic Crystal.
			    for (Player player: instance.getPlayersInside()) {
				    if (player.isOnline()) {
						dropItems.add(DropRegistrationService.getInstance().regDropItem(index++, player.getObjectId(), npcId, 188072800, 1));
					}
				}
			break;
			case 839629: //Suffering Primeth Cubic Crystal.
			    for (Player player: instance.getPlayersInside()) {
				    if (player.isOnline()) {
						dropItems.add(DropRegistrationService.getInstance().regDropItem(index++, player.getObjectId(), npcId, 188072801, 1));
					}
				}
			break;
			case 839630: //Berserking Tarukan Cubic Crystal.
			    for (Player player: instance.getPlayersInside()) {
				    if (player.isOnline()) {
						dropItems.add(DropRegistrationService.getInstance().regDropItem(index++, player.getObjectId(), npcId, 188072802, 1));
					}
				}
			break;
			case 839631: //Berserking Maddened Frigida Cubic Crystal.
			    for (Player player: instance.getPlayersInside()) {
				    if (player.isOnline()) {
						dropItems.add(DropRegistrationService.getInstance().regDropItem(index++, player.getObjectId(), npcId, 188072803, 1));
					}
				}
			break;
        }
	...
	public void onEnterInstance(final Player player) {
		...
		if (player.getRace() == Race.ELYOS) {
			ClassChangeService.onUpdateQuest62890(player);
		} else {
			ClassChangeService.onUpdateQuest72890(player);
		} if (spawnRace == null) {
	...
	public void onDie(Npc npc) {
		...
		switch (npc.getObjectTemplate().getTemplateId()) {
			case 858365: //Berserking Yastikan.
			case 858366: //Berserking Varakan.
			    primethForgeBattle();
			break;
			case 858367: //Suffering Primeth.
				sp(650019, 789.0000f, 842.0000f, 694.0000f, (byte) 30, 10000, 0, null); //Liberated Primeth.
				sp(839629, 789.0000f, 870.0000f, 694.0000f, (byte) 91, 2000, 0, null); //Suffering Primeth Cubic Crystal.
			break;
			case 858368: //Berserking Tarukan.
				...
				sp(839630, 1022.5268f, 1087.4364f, 624.9460f, (byte) 60, 2000, 0, null); //Berserking Tarukan Cubic Crystal.
			break;
			case 858369: //Berserking Frigida.
				...
				sp(839631, 1324.8818f, 1090.2180f, 374.4916f, (byte) 60, 3000, 0, null); //Berserking Frigida Cubic Crystal.
			break;
	...
	public void handleUseItemFinish(Player player, Npc npc) {
		switch (npc.getNpcId()) {
			...
			case 837096: //Primeth Furnace.
				primethForgeH(player, 790.0000f, 768.0000f, 292.0000f, (byte) 49);
			break;
		}
	}
	
	protected void primethForgeH(Player player, float x, float y, float z, byte h) {
                            ...
                            ThreadPoolManager.getInstance().schedule(new Runnable() {
								...
								public void run() {
									if (primeth.getNpcId() == 655465) {
										...
										spawn(206715, 1043.0000f, 1090.0000f, 624.0000f, (byte) 0);
										instance.doOnAllPlayers(new Visitor<Player>() {
											@Override
											public void visit(Player player) {
												if (player.isOnline()) {
													PacketSendUtility.sendSys3Message(player, "\uE005", "Approach the forge to descend below!!!");
												}
											}
										});
									}
	...
	private boolean primethForgeBattle() {
		Npc boss1 = getNpc(858365); //Berserking Yastikan.
		Npc boss2 = getNpc(858366); //Berserking Varakan.
		if (isDead(boss1) && isDead(boss2)) {
			doors.get(206).setOpen(true);
			//Use the open entrance to move to the next area.
			sendMsgByRace(1402781, Race.PC_ALL, 2000);
			sp(839628, 916.0000f, 697.0000f, 749.0000f, (byte) 0, 3000, 0, null); //Berserking Yastikan-Berserking Varakan Cubic Crystal.
			return true;
		}
		return false;
	}
