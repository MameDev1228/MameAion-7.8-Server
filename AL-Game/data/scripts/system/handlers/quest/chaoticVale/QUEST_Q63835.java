...
public class QUEST_Q63835 extends QuestHandler
{
	private final static int questId = 63835;
	...
	public void register() {
		qe.registerQuestItem(182217006, questId);
		qe.registerQuestItem(182217007, questId);
		qe.registerQuestItem(182217008, questId);
			...
			if (targetId == 806978) {
				switch (env.getDialog()) {
                    case START_DIALOG: {
						...
						giveQuestItem(env, 182217006, 1);
						giveQuestItem(env, 182217007, 1);
						return sendQuestStartDialog(env, 182217008, 1);
    ...
    public HandlerResult onItemUseEvent(QuestEnv env, final Item item) {
        ...
        if (id == 182217006) {
			if (var == 0 && player.isInsideZone(ZoneName.get("IDTAGWAR_ITEMUSEAREA_Q63835A"))) {
				...
				giveQuestItem(env, 182217007, 1);
				removeQuestItem(env, 182217006, 1);
				return HandlerResult.SUCCESS;
			}
		} else if (id == 182217007) {
			if (var == 1 && player.isInsideZone(ZoneName.get("IDTAGWAR_ITEMUSEAREA_Q63835B"))) {
				qs.setQuestVar(2);
				updateQuestStatus(env);
				giveQuestItem(env, 182217008, 1);
				removeQuestItem(env, 182217007, 1);
				return HandlerResult.SUCCESS;
			}
		} else if (id == 182217008) {
			if (var == 2 && player.isInsideZone(ZoneName.get("IDTAGWAR_ITEMUSEAREA_Q63835C"))) {
				qs.setQuestVar(3);
				qs.setStatus(QuestStatus.REWARD);
				updateQuestStatus(env);
				removeQuestItem(env, 182217008, 1);
				return HandlerResult.SUCCESS;
			}
		}
