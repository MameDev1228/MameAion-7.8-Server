			...
			switch (siegeLocationId) {
				...
				case 2021: //Altar Of Avarice.
				    if (loc.getRace() == SiegeRace.ASMODIANS) {
						DynamicRiftService.getInstance().stopDynamicRift(3);
						World.getInstance().doOnAllPlayers(new Visitor<Player>() {
							@Override
							public void visit(Player player) {
								//Silentera Westgate, the entrance from Inggison to Silentera Canyon, has closed.
								PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_FIELDABYSS_LIGHTUNDERPASS_DESPAWN, 0);
							}
						});
					} else if (loc.getRace() == SiegeRace.ELYOS) {
						DynamicRiftService.getInstance().startDynamicRift(3);
						World.getInstance().doOnAllPlayers(new Visitor<Player>() {
							@Override
							public void visit(Player player) {
								//Silentera Westgate, the entrance from Inggison to Silentera Canyon, has opened.
								PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_FIELDABYSS_LIGHTUNDERPASS_SPAWN, 0);
							}
						});
				...
				case 3021: //Crimsom Temple.
				    if (loc.getRace() == SiegeRace.ASMODIANS) {
						DynamicRiftService.getInstance().startDynamicRift(4);
						World.getInstance().doOnAllPlayers(new Visitor<Player>() {
							@Override
							public void visit(Player player) {
								//Silentera Eastgate, the entrance from Gelkmaros to Silentera Canyon, has opened.
								PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_FIELDABYSS_DARKUNDERPASS_SPAWN, 0);
							}
						});
					} else if (loc.getRace() == SiegeRace.ELYOS) {
						DynamicRiftService.getInstance().stopDynamicRift(4);
						World.getInstance().doOnAllPlayers(new Visitor<Player>() {
							@Override
							public void visit(Player player) {
								//Silentera Eastgate, the entrance from Gelkmaros to Silentera Canyon, has closed.
								PacketSendUtility.playerSendPacketTime(player, SM_SYSTEM_MESSAGE.STR_FIELDABYSS_DARKUNDERPASS_DESPAWN, 0);
							}
						});
					} else if (loc.getRace() == SiegeRace.BALAUR) {
						DynamicRiftService.getInstance().stopDynamicRift(4);
