...
public class SimpleRootEffect extends EffectTemplate
{
	...
	public void calculate(Effect effect) {
		if (effect.getEffected().getEffectController().hasAbnormalEffect(8224) &&
			...
			effect.getEffected().getEffectController().hasAbnormalEffect(9220) &&
			effect.getEffected().getEffectController().hasAbnormalEffect(9317) &&
			effect.getEffected().getEffectController().hasAbnormalEffect(9338) &&
			effect.getEffected().getEffectController().hasAbnormalEffect(9380) &&
			effect.getEffected().getEffectController().hasAbnormalEffect(9404)) {
			return;
        }
	...
	public void startEffect(final Effect effect) {
		...
		Vector3f closestCollision = GeoService.getInstance().getClosestCollision(effected, effector.getX() + x1, effector.getY() + y1, effected.getZ() + z, false, intentions);
