...
public class QUEST_Q70608 extends QuestHandler
{
	private final static int questId = 70608;
	private final static int[] npcs = {840663, 806989, 839956, 840672};
	...
	public void register() {
		...
		qe.registerQuestItem(182217108, questId);
		qe.registerOnEnterZoneMissionEnd(questId);
		qe.registerOnEnterZone(ZoneName.get("GELKMAROS_FORTRESS_220070000"), questId);
		qe.registerOnEnterZone(ZoneName.get("SILENTERA_EASTGATE_600010000"), questId);
	...
	public HandlerResult onItemUseEvent(final QuestEnv env, Item item) {
		...
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				removeQuestItem(env, 182217108, 1);
			}
		}, 3000);
    ...
    public boolean onEnterZoneEvent(QuestEnv env, ZoneName zoneName) {
        ...
        if (qs != null && qs.getStatus() == QuestStatus.START) {
			...
			} else if (zoneName == ZoneName.get("SILENTERA_EASTGATE_600010000")) {
				if (var == 4) {
					qs.setQuestVar(5);
					updateQuestStatus(env);
					return true;
