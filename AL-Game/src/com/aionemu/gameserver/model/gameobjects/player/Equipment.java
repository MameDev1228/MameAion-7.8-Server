...
public class Equipment
{
	...
	private Item equip(long itemSlotToEquip, Item item) {
		if (item.getOptionalSocket() == -1) {
			//Equipment can only be enchanted after tuning.
			PacketSendUtility.sendPacket(owner, SM_SYSTEM_MESSAGE.STR_MSG_CANT_ENCHANT_IDENTIFY);
			return null;
		}
		synchronized (equipment) {
	...
	private boolean hasDualWieldingSkills() {
		return owner.getSkillList().isSkillPresent(55) ||
		owner.getSkillList().isSkillPresent(143) ||
		owner.getSkillList().isSkillPresent(144) ||
		owner.getSkillList().isSkillPresent(171) ||
			...
			if (itemInRightHand != null) { // main hand is already occupied
				...
				if (mainIsTwoHand) {
					...
					if (addingLeftHand && itemInLeftHand != null) {
						...
						// for dual weapons, they occupy two slots, and not player can equip two one-handed weapons 4.9 - 5.1
	...
	public FastList<Item> getEquippedForApparence() {
		...
		for (Item item : equipment.values()) {
			...
			if (!ItemSlot.isStigma(slot)) {
				if (slot <= ItemSlot.GLYPH.getSlotIdMask()) {
					if (item.getItemTemplate().isTwoHandWeapon()) {
						if (twoHanded != null) {
							continue;
						}
	...
	public void usePowerShard(Item powerShardItem, int count) {
		...
		if (powerShardItem.getItemCount() <= 0) {
			...
			if (powerShardStacks.size() != 0) {
				...
				PacketSendUtility.sendPacket(owner, SM_SYSTEM_MESSAGE.STR_WEAPON_BOOST_CANNOT_USE_WARNING);
				owner.unsetState(CreatureState.POWERSHARD);
			}
		}
	}
	...
	public long increaseEquippedItemCount(Item item, long count) {
		//Power Shards 6.x can be increased.
	...
	private void decreaseEquippedItemCount(int itemObjId, int count) {
		...
		//Power Shards 6.x can be decreased.
