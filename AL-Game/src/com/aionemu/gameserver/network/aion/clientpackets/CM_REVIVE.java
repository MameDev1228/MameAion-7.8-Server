...
public class CM_REVIVE extends AionClientPacket
{
	...
	protected void runImpl() {
		...
		switch (reviveType) {
			...
			break;
			case ORBIS_REVIVE:
				if (activePlayer.getWorldId() == 800060000) { //Demaha.
					PlayerReviveService.startOrbisRevive(activePlayer);
				} else {
					PlayerReviveService.bindRevive(activePlayer);
					PacketSendUtility.sendMessage(activePlayer, "You can resurrect only in <Orbis Training Center> in Demaha");
				}
			break;
			case LUNA_REVIVE:
				LunaShopService.getInstance().lunaRevive(activePlayer);
			break;
			default:
			break;
		}
