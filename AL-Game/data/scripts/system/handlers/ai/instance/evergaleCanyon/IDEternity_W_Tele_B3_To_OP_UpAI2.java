...
public class IDEternity_W_Tele_B3_To_OP_UpAI2 extends NpcAI2
{
	...
	private void IDEternity_W_Tele_B3_To_OP_L_Up() {
		...
		deleteNpcs(instance1.getNpcs(835290));
		deleteNpcs(instance1.getNpcs(835454));
		spawn(835278, 746.0000f, 850.0000f, 347.0000f, (byte) 0, 257);
		spawn(835453, 746.0000f, 850.0000f, 347.0000f, (byte) 0, 323);
    }
	
	private void IDEternity_W_Tele_B3_To_OP_D_Up() {
		getOwner().getController().onDelete();
		WorldMapInstance instance2 = getPosition().getWorldMapInstance();
		deleteNpcs(instance2.getNpcs(835278));
	...
	private void announceTele01E() {
		getPosition().getWorldMapInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				if (player.isOnline()) {
					//B3의 순간이동 장치를 천족이 점령해 이용 가능 합니다.
	...
	private void announceTele02A() {
		getPosition().getWorldMapInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				if (player.isOnline()) {
					//B3의 순간이동 장치를 마족이 점령해 이용 가능 합니다.
