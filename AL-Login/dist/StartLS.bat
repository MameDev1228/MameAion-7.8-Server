...
TITLE MameAion 7.8.0 - Login Server / ログインサーバー
:START
CLS
echo ================================================================================
echo   MameAion 7.8.0  Login Server / ログインサーバー
...
ECHO.
ECHO [起動] MameAion 7.8.0 Login Server / %MODE%
"%JAVA_EXE%" %JDK25_COMPAT_OPTS% %ENCODING_OPTS% %JAVA_OPTS% -cp "./libs/*;AL-Login.jar" com.aionemu.loginserver.LoginServer
...
:ERROR
ECHO.
ECHO [停止] Login Server が異常終了しました。
ECHO.
PAUSE
EXIT /b 1
:END
ECHO.
ECHO [停止] Login Server は正常終了しました。
ECHO.
PAUSE
EXIT /b 0
