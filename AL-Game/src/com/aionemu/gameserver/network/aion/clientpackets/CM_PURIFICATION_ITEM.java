...
public class CM_PURIFICATION_ITEM extends AionClientPacket
{
	...
	protected void runImpl() {
		...
		if (player == null) {
			return;
		} if (!ItemUpgradeService.checkItemUpgrade(player, baseItem, choice)) {
			return;
		}
		Item resultItem = ItemService.newItem(resultItemId, 1, null, 0, 0, 0);
		ItemService.makeUpgradeItem(baseItem, resultItem);
		if (!ItemUpgradeService.decreaseMaterial(player, baseItem, choice)) {
			return;
		}
		ItemService.addItem(player, resultItem);
		//%0 has been completely upgraded, and you have gained %1.
		PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_ITEM_UPGRADE_MSG_UPGRADE_SUCCESS(new DescriptionId(baseItem.getNameId()), new DescriptionId(resultItem.getNameId())));
	}
}