...
public class InstanceEntryService
{
    public void onResetInstanceEntry(Player player, int worldId, InstanceEntryCostEnum type) {
        ...
        } else if (type == InstanceEntryCostEnum.LUNA && player.getLunaAccount() >= time.getLuna()) {
    ...
    public static InstanceEntryService getInstance() {
        return InstanceEntryServiceHolder.INSTANCE;
    }
	
    private static class InstanceEntryServiceHolder {
        private static final InstanceEntryService INSTANCE = new InstanceEntryService();
    }
}