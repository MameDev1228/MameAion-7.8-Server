...
public class PlayerService
{
	...
	public static Player getPlayer(int playerObjId, Account account) {
		...
		PlayerCommonData pcd = DAOManager.getDAO(PlayerDAO.class).loadPlayerCommonData(playerObjId);
		...
		DAOManager.getDAO(LoginEventDAO.class).load(player);
	...
	public static Player newPlayer(PlayerCommonData playerCommonData, PlayerAppearance playerAppearance, Account account) {
		...
		MonsterCoreService.getInstance().onCreatePlayer(newPlayer);
		...
		if (playerCreationData != null) {
			...
			if (playerCommonData.getRace() == Race.ASMODIANS) {
				Item item = ItemFactory.newItem(102050065, 1);
				playerInventory.onLoadHandler(item);
			} else {
				Item item = ItemFactory.newItem(102050064, 1);
				playerInventory.onLoadHandler(item);
			}
		}
	...
	public static Player newPlayerJumping(PlayerCommonData playerCommonData, PlayerAppearance playerAppearance, Account account) {
		PlayerInitialData playerInitialData = DataManager.PLAYER_INITIAL_DATA;
		LocationData ld = playerInitialData.getJumpingSpawnLocation(playerCommonData.getRace());
		WorldPosition position = World.getInstance().createPosition(ld.getMapId(), ld.getX(), ld.getY(), ld.getZ(), ld.getHeading(), 0);
		playerCommonData.setPosition(position);
		Player newPlayerJumping = new Player(new PlayerController(), playerCommonData, playerAppearance, account);
		newPlayerJumping.setSkillList(new PlayerSkillList());
		SkillLearnService.addNewSkills(newPlayerJumping);
		SkillLearnService.addMissingSkills(newPlayerJumping);
		//Jumping Completed Craft.
		newPlayerJumping.getSkillList().addSkill(newPlayerJumping, 30002, 499);
		newPlayerJumping.getSkillList().addSkill(newPlayerJumping, 40009, 1);
		newPlayerJumping.getSkillList().addSkill(newPlayerJumping, 40011, 300);
		MonsterCoreService.getInstance().onCreatePlayer(newPlayerJumping);
		PlayerCreationData playerCreationData = playerInitialData.getPlayerCreationData(playerCommonData.getPlayerClass());
		Storage playerInventory = new PlayerStorage(StorageType.CUBE);
		Storage regularWarehouse = new PlayerStorage(StorageType.REGULAR_WAREHOUSE);
		Storage accountWarehouse = new PlayerStorage(StorageType.ACCOUNT_WAREHOUSE);
		Equipment equipment = new Equipment(newPlayerJumping);
		if (playerCreationData != null) {
			List<ItemType> items = playerCreationData.getItems();
			for (ItemType itemType : items) {
				if (itemType.getRace() != Race.PC_ALL && itemType.getRace() != newPlayerJumping.getRace()) {
					continue;
				}
				int itemId = itemType.getTemplate().getTemplateId();
				Item item = ItemFactory.newItem(itemId, itemType.getCount());
		...
		newPlayerJumping.setStorage(playerInventory, StorageType.CUBE);
		newPlayerJumping.setStorage(regularWarehouse, StorageType.REGULAR_WAREHOUSE);
		newPlayerJumping.setStorage(accountWarehouse, StorageType.ACCOUNT_WAREHOUSE);
		newPlayerJumping.setEquipment(equipment);
		newPlayerJumping.setMailbox(new Mailbox(newPlayerJumping));
		//Jumping Completed Mission.
		switch (newPlayerJumping.getRace()) {
			case ELYOS:
				ClassChangeService.completeQuest(newPlayerJumping, 1006);
				ClassChangeService.completeQuest(newPlayerJumping, 14043);
				ClassChangeService.completeQuest(newPlayerJumping, 60000);
				ClassChangeService.completeQuest(newPlayerJumping, 60001);
				ClassChangeService.completeQuest(newPlayerJumping, 60002);
				ClassChangeService.completeQuest(newPlayerJumping, 60003);
				ClassChangeService.completeQuest(newPlayerJumping, 60004);
				ClassChangeService.completeQuest(newPlayerJumping, 60005);
				ClassChangeService.completeQuest(newPlayerJumping, 60006);
				ClassChangeService.completeQuest(newPlayerJumping, 60007);
				ClassChangeService.completeQuest(newPlayerJumping, 60008);
				ClassChangeService.completeQuest(newPlayerJumping, 60009);
				ClassChangeService.completeQuest(newPlayerJumping, 60100);
				ClassChangeService.completeQuest(newPlayerJumping, 60101);
				ClassChangeService.completeQuest(newPlayerJumping, 60110);
				ClassChangeService.completeQuest(newPlayerJumping, 60200);
				ClassChangeService.completeQuest(newPlayerJumping, 60201);
				ClassChangeService.completeQuest(newPlayerJumping, 60202);
				ClassChangeService.completeQuest(newPlayerJumping, 60203);
				ClassChangeService.completeQuest(newPlayerJumping, 60204);
				ClassChangeService.completeQuest(newPlayerJumping, 60205);
				ClassChangeService.completeQuest(newPlayerJumping, 60206);
				ClassChangeService.completeQuest(newPlayerJumping, 60207);
				ClassChangeService.completeQuest(newPlayerJumping, 60208);
				ClassChangeService.completeQuest(newPlayerJumping, 61601);
				ClassChangeService.completeQuest(newPlayerJumping, 61603);
			break;
			case ASMODIANS:
				ClassChangeService.completeQuest(newPlayerJumping, 2008);
				ClassChangeService.completeQuest(newPlayerJumping, 24043);
				ClassChangeService.completeQuest(newPlayerJumping, 70000);
				ClassChangeService.completeQuest(newPlayerJumping, 70001);
				ClassChangeService.completeQuest(newPlayerJumping, 70002);
				ClassChangeService.completeQuest(newPlayerJumping, 70003);
				ClassChangeService.completeQuest(newPlayerJumping, 70004);
				ClassChangeService.completeQuest(newPlayerJumping, 70005);
				ClassChangeService.completeQuest(newPlayerJumping, 70006);
				ClassChangeService.completeQuest(newPlayerJumping, 70007);
				ClassChangeService.completeQuest(newPlayerJumping, 70008);
				ClassChangeService.completeQuest(newPlayerJumping, 70009);
				ClassChangeService.completeQuest(newPlayerJumping, 70100);
				ClassChangeService.completeQuest(newPlayerJumping, 70101);
				ClassChangeService.completeQuest(newPlayerJumping, 70110);
				ClassChangeService.completeQuest(newPlayerJumping, 70200);
				ClassChangeService.completeQuest(newPlayerJumping, 70201);
				ClassChangeService.completeQuest(newPlayerJumping, 70202);
				ClassChangeService.completeQuest(newPlayerJumping, 70203);
				ClassChangeService.completeQuest(newPlayerJumping, 70204);
				ClassChangeService.completeQuest(newPlayerJumping, 70205);
				ClassChangeService.completeQuest(newPlayerJumping, 70206);
				ClassChangeService.completeQuest(newPlayerJumping, 70207);
				ClassChangeService.completeQuest(newPlayerJumping, 70208);
				ClassChangeService.completeQuest(newPlayerJumping, 71601);
				ClassChangeService.completeQuest(newPlayerJumping, 71603);
			break;
		}
		account.setJumping(0);
		account.setMembership((byte) 0);
		DAOManager.getDAO(AccountDAO.class).SaveJumpingCountOnAccount(account);
		for (int petBagId = StorageType.PET_BAG_MIN; petBagId <= StorageType.PET_BAG_MAX; petBagId++) {
			Storage petBag = new PlayerStorage(StorageType.getStorageTypeById(petBagId));
			newPlayerJumping.setStorage(petBag, StorageType.getStorageTypeById(petBagId));
		} for (int houseWhId = StorageType.HOUSE_WH_MIN; houseWhId <= StorageType.HOUSE_WH_MAX; houseWhId++) {
			StorageType whType = StorageType.getStorageTypeById(houseWhId);
			if (whType != null) {
				Storage cabinet = new PlayerStorage(whType);
				newPlayerJumping.setStorage(cabinet, StorageType.getStorageTypeById(houseWhId));
			}
		}
		playerInventory.setPersistentState(PersistentState.UPDATE_REQUIRED);
		equipment.setPersistentState(PersistentState.UPDATE_REQUIRED);
		return newPlayerJumping;
	}
