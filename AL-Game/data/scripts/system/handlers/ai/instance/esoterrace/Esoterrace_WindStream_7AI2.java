...
@AIName("Esoterrace_WindStream_7")
public class Esoterrace_WindStream_7AI2 extends NpcAI2
{
	@Override
	...
	private void startWindStream(final Npc npc) {
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                Npc npc2 = (Npc) spawn(302850000, 703114, 392.0000f, 543.0000f, 318.0000f, (byte) 18, 0, 1);
				windStreamAnnounce(npc2, 1);
				WorldMapInstance instance = getPosition().getWorldMapInstance();
				deleteNpcs(instance.getNpcs(703113));
                spawn(703114, 392.0000f, 543.0000f, 318.0000f, (byte) 18);
                PacketSendUtility.broadcastPacket(npc2, new SM_WINDSTREAM_ANNOUNCE(1, 302850000, 456, 1));
                if (npc2 != null) {
    ...
    private void windStreamAnnounce(final Npc npc, final int state) {
        ...
        npc.getPosition().getWorld().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
                PacketSendUtility.sendPacket(player, new SM_WINDSTREAM_ANNOUNCE(1, 302850000, 456, state));
            }
        });
    }
