...
public class HallOfTenacityReward extends InstanceReward<HallOfTenacityPlayerReward>
{
	...
	public void portToArena(Player player) {
        if (player.getHOTVSId() == 0) {
            TeleportService2.teleportTo(player, 302590000, instanceId, myBattlePosition.getX(), myBattlePosition.getY(), myBattlePosition.getZ());
        } else if (player.getHOTVSId() == 1) {
            TeleportService2.teleportTo(player, 302590000, instanceId, opponentBattlePosition.getX(), opponentBattlePosition.getY(), opponentBattlePosition.getZ());
        }
    }
	...
	//TODO VSId
	public void setCoupleSlotForBattle8() {
		int size = 7; // 16/2=8 (packet first slot count from 0 to 7)
