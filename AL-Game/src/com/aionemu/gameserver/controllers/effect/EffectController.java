...
public class EffectController
{
	...
	public void addEffect(Effect nextEffect) {
		...
		try {
			...
			//Max 3 Chants Effect
            if (nextEffect.isToggle() && !nextEffect.isAethertechEffect(nextEffect.getSkillTemplate().getSkillId())) {
				int mts = 1;
                if (nextEffect.getSkillSubType() == SkillSubType.CHANT) {
                    mts = 3;
                } else if (isSongweaverEffect(nextEffect.getSkillId())) {
                    mts = 2;
                } else if (isToggleRanger(nextEffect.getSkillId())) {
                    mts = 3;
                } else {
                    mts = 1;
            ...
            if (nextEffect.isRangerBuff()) {
                ...
                if (rangerBuff.size() >= 3) {
                    Iterator<Effect> rangerIter = rangerBuff.iterator();
                    rangerIter.next().endEffect();
                }
            }
			//Max 1 Vandal Channelling.
            if (nextEffect.isVandalChannelling()) {
                Collection<Effect> vandalChannelling = this.getVandalChannelling();
                if (vandalChannelling.size() >= 1) {
                    Iterator<Effect> vandalIter = vandalChannelling.iterator();
                    vandalIter.next().endEffect();
                }
            } if (!nextEffect.isPassive()) {
	...
	public boolean isSongweaverEffect(int skillId) {
		switch (skillId) {
			//Inspiration.
			case 4538:
			//Exultation.
	...
	public Collection<Effect> getVandalChannelling() {
        return Collections2.filter(abnormalEffectMap.values(), new Predicate<Effect>() {
            @Override
            public boolean apply(Effect effect) {
                return effect.isVandalChannelling();
            }
        });
    }
