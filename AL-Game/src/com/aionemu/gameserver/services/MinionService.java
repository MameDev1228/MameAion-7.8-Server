...
import com.aionemu.gameserver.network.aion.serverpackets.SM_MINION;
...
public class MinionService
{
	...
	public void init() {
		...
		mb = new MinionBuff();
		log.info("Minion System initialized!");
	}
			...
			public void run() {
				...
				if (it.getMinionTicket()) {
					if (it.isMinionCashContract()) {
						minionId = minionRnd;
						MinionTemplate mt = DataManager.MINION_DATA.getMinionTemplate(minionId);
						grade = mt.getGrade();
						name = mt.getName();
					} else {
						for (Integer asd : minions) {
							MinionTemplate mt = DataManager.MINION_DATA.getMinionTemplate(asd);
							if (mt.getLevel() == 1) {
								minionId = minionRnd;
				...
				if (player.getRace() == Race.ELYOS) {
					...
					QuestState qs25545 = player.getQuestStateList().getQuestState(25545);
					if (qs25545 != null && qs25545.getStatus() == QuestStatus.START && qs25545.getQuestVarById(0) == 0) {
				...
				if (mcd != null) {
					PacketSendUtility.sendPacket(player, new SM_MINION(2, mcd, 0));
				}
	...
	public void spawnMinion(Player player, int minionObjId) {
		...
		if (player.getMinion() != null && player.getMinion().getObjectId() == minionObjId) {
			// Idempotent refresh. CM_LEVEL_READY / known-list refresh can be fired more than once;
			// do not despawn/reapply the same minion or its stat buff.
	...
	public void despawnMinion(Player player, int minionObjId) {
		...
		DAOManager.getDAO(PlayerDAO.class).storePlayer(player);
		mb.end(player);
		PacketSendUtility.broadcastPacketAndReceive(player, new SM_MINION(7, minionCommonData));
	}
	...
	public void onLoggedIn(Player player) {
		PacketSendUtility.sendPacket(player, new SM_MINION(0));
		PacketSendUtility.sendPacket(player, new SM_MINION(1, player.getMinionList().getMinions()));
		PacketSendUtility.sendPacket(player, new SM_MINION(11, player));
		PacketSendUtility.sendPacket(player, new SM_MINION(13));
		PacketSendUtility.sendPacket(player, new SM_MINION(14));
		if (player.getCommonData().getLastMinion() != 0) {
			...
			PacketSendUtility.sendPacket(player, new SM_MINION(11, 86400000 * 30));
		}
		player.getMinionList().updateMinionsList();
	}
	...
	public void addMinionGrowth(Player player, int exp) {
		int finalExp = 0;
		MinionCommonData mcd = player.getMinionList().getMinion(player.getMinion().getObjectId());
		MinionTemplate mt = DataManager.MINION_DATA.getMinionTemplate(mcd.getMinionId());
		int growth = mcd.getGrowthPoints();
		finalExp = growth + exp;
		...
		PacketSendUtility.sendPacket(player, new SM_MINION(8, mcd, 0));
		...
		for (MinionCommonData list : player.getMinionList().getMinions()) {
			for (int sacrifices : sacrificeMinions) {
				if (list.getObjectId() == sacrifices) {
					MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(list.getMinionId());
					kinahCost += mt1.getGrowthCost();
					...
					finalExp += mt1.getGrowthPoints();
					player.getMinionList().disMissMinion(list.getMinionId());
					PacketSendUtility.sendPacket(player, new SM_MINION(3, list, 0));
	...
	public void EnergyRecharge(Player player, int isAuto) {
		...
		if (player.getCommonData().getMinionEnergy() >= 5000) {
			//Minion Energy has completed charging.
	...
	public void lockMinion(Player player, int objId, int lock) {
		...
		PacketSendUtility.sendPacket(player, new SM_MINION(5, mcd, 0));
	...
	public void EvolveMinion(Player player, int objId) {
		...
		if (player.getInventory().getItemCountByItemId(mt.getEvolved().getItemId()) < mt.getEvolved().getEvolvedNum()) {
			//You don't have enough items for evolution.
			...
			//You don't have enough Kinah for evolution.
		...
		PacketSendUtility.sendPacket(player, new SM_MINION(3, mcd, 0));
		//Minion %0 has evolved to stage %1.
		...
		MinionTemplate mt2 = DataManager.MINION_DATA.getMinionTemplate(minionId);
		grade = mt2.getGrade();
		MinionCommonData mcd3 = player.getMinionList().addNewMinion(player, minionId, name, grade, 1, 0);
		if (mcd != null) {
			player.getMinionList().updateMinionsList();
			PacketSendUtility.sendPacket(player, new SM_MINION(2, mcd3, 0));
		}
	}
	...
	public void CombineMinion(final Player player, final int conbine1, final int conbine2, final int conbine3, final int conbine4) {
		...
		final String name6;
		if (combineSuccess) {
			PacketSendUtility.sendPacket(player, new SM_MINION(3, mcd1, 3));
			player.getMinionList().disMissMinion(mcd1.getMinionId());
			dismissMinion(player, mcd1.getObjectId());
			PacketSendUtility.sendPacket(player, new SM_MINION(3, mcd2, 3));
			player.getMinionList().disMissMinion(mcd2.getMinionId());
			dismissMinion(player, mcd2.getObjectId());
			PacketSendUtility.sendPacket(player, new SM_MINION(3, mcd3, 3));
			player.getMinionList().disMissMinion(mcd3.getMinionId());
			dismissMinion(player, mcd3.getObjectId());
			PacketSendUtility.sendPacket(player, new SM_MINION(3, mcd4, 3));
			player.getMinionList().disMissMinion(mcd4.getMinionId());
			dismissMinion(player, mcd4.getObjectId());
		} else {
			PacketSendUtility.sendPacket(player, new SM_MINION(3, mcd1, 2));
			player.getMinionList().disMissMinion(mcd1.getMinionId());
			dismissMinion(player, mcd1.getObjectId());
			PacketSendUtility.sendPacket(player, new SM_MINION(3, mcd2, 2));
			player.getMinionList().disMissMinion(mcd2.getMinionId());
			dismissMinion(player, mcd2.getObjectId());
			PacketSendUtility.sendPacket(player, new SM_MINION(3, mcd3, 2));
			player.getMinionList().disMissMinion(mcd3.getMinionId());
			dismissMinion(player, mcd3.getObjectId());
			PacketSendUtility.sendPacket(player, new SM_MINION(3, mcd4, 2));
			player.getMinionList().disMissMinion(mcd4.getMinionId());
			dismissMinion(player, mcd4.getObjectId());
		} if (combineSuccess && !isKerub(mt.getId()) && !isRankA(mt.getId())) {
			if (mt.getGradeId() == 3) { //C
				final ItemMinionList minionList1 = DataManager.ITEM_MINION_LIST.getMinionList(31); //Grade B
				final int minionRnd = RndArray.get(minionList1.getMinionId());
				minionId = minionRnd;
				MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(minionId);
				grade = mt1.getGrade();
				name = mt1.getName();
				MinionCommonData mcd = player.getMinionList().addNewMinion(player, minionId, name, grade, 1, 0);
				if (mcd != null) {
					PacketSendUtility.sendPacket(player, new SM_MINION(2, mcd, 2));
				}
			} else if (mt.getGradeId() == 2) { //B
				final ItemMinionList minionList1 = DataManager.ITEM_MINION_LIST.getMinionList(32); //Grade A
				final int minionRnd = RndArray.get(minionList1.getMinionId());
				minionId2 = minionRnd;
				MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(minionId2);
				grade2 = mt1.getGrade();
				name2 = mt1.getName();
				MinionCommonData mcd = player.getMinionList().addNewMinion(player, minionId2, name2, grade2, 1, 0);
				if (mcd != null) {
					PacketSendUtility.sendPacket(player, new SM_MINION(2, mcd, 2));
				}
			}
			...
			if (mt.getGradeId() == 3) { //C
				ItemMinionList minionList1 = DataManager.ITEM_MINION_LIST.getMinionList(30); //Grade C
				int minionRnd = RndArray.get(minionList1.getMinionId());
				minionId = minionRnd;
				MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(minionId);
				grade = mt1.getGrade();
				name = mt1.getName();
				MinionCommonData mcd = player.getMinionList().addNewMinion(player, minionId, name, grade, 1, 0);
				PacketSendUtility.sendPacket(player, new SM_MINION(2, mcd, 3));
			} else if (mt.getGradeId() == 2) { //B
				ItemMinionList minionList1 = DataManager.ITEM_MINION_LIST.getMinionList(31); //Grade B
				int minionRnd = RndArray.get(minionList1.getMinionId());
				minionId2 = minionRnd;
				MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(minionId2);
				grade2 = mt1.getGrade();
			...
			if (mt.getGradeId() == 4) { //D
				ItemMinionList minionList1 = DataManager.ITEM_MINION_LIST.getMinionList(34); //Grade C
				int minionRnd = RndArray.get(minionList1.getMinionId());
				minionId3 = minionRnd;
				MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(minionId3);
				grade3 = mt1.getGrade();
				name3 = mt1.getName();
				MinionCommonData mcd = player.getMinionList().addNewMinion(player, minionId3, name3, grade3, 1, 0);
				PacketSendUtility.sendPacket(player, new SM_MINION(2, mcd, 3));
			} else if (mt.getGradeId() == 3) { //C
				ItemMinionList minionList1 = DataManager.ITEM_MINION_LIST.getMinionList(35); //Grade B
				int minionRnd = RndArray.get(minionList1.getMinionId());
				minionId4 = minionRnd;
				MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(minionId4);
				grade4 = mt1.getGrade();
				name4 = mt1.getName();
				MinionCommonData mcd = player.getMinionList().addNewMinion(player, minionId4, name4, grade4, 1, 0);
				PacketSendUtility.sendPacket(player, new SM_MINION(2, mcd, 3));
			} else if (mt.getGradeId() == 2) { //B
				ItemMinionList minionList1 = DataManager.ITEM_MINION_LIST.getMinionList(36); //Grade A
				int minionRnd = RndArray.get(minionList1.getMinionId());
				minionId5 = minionRnd;
				MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(minionId5);
				grade5 = mt1.getGrade();
				name5 = mt1.getName();
				MinionCommonData mcd = player.getMinionList().addNewMinion(player, minionId5, name5, grade5, 1, 0);
				PacketSendUtility.sendPacket(player, new SM_MINION(2, mcd, 3));
			}
		} else {
			if (mt.getGradeId() == 4) { //D
				ItemMinionList minionList1 = DataManager.ITEM_MINION_LIST.getMinionList(33); //Grade D
				int minionRnd = RndArray.get(minionList1.getMinionId());
				minionId3 = minionRnd;
				MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(minionId3);
				grade3 = mt1.getGrade();
		...
		if (combineSuccess && isRankA(mt.getId())) {
			if (mt.getGradeId() == 4) { //C-B-A
				final ItemMinionList minionList1 = DataManager.ITEM_MINION_LIST.getMinionList(37); //Grade S
				final int minionRnd = RndArray.get(minionList1.getMinionId());
				minionId6 = minionRnd;
				MinionTemplate mt1 = DataManager.MINION_DATA.getMinionTemplate(minionId6);
				grade6 = mt1.getGrade();
				name6 = mt1.getName();
				MinionCommonData mcd = player.getMinionList().addNewMinion(player, minionId6, name6, grade6, 1, 0);
				if (mcd != null) {
					PacketSendUtility.sendPacket(player, new SM_MINION(2, mcd, 2));
				}
			}
	...
	public boolean isKerub(int minionId) {
		if (minionId == 980010 || minionId == 980011 || minionId == 980012 || minionId == 980013) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isRankA(int minionId) {
		if (minionId == 980063 || minionId == 980073 || minionId == 980085 || minionId == 980089 || minionId == 980093) {
			return true;
		} else {
			return false;
		}
	...
	public void renameMinion(Player player, int objId, String name) {
		...
		PacketSendUtility.broadcastPacketAndReceive(player, new SM_MINION(4, mcd, 0));
	...
	public void activateMinionFunction(Player player) {
		long leftTime = System.currentTimeMillis() + (86400000 * 30); // + 30 Days
	...
	public void activateLoot(final Player player, int minionObj, final boolean activate) {
		...
		PacketSendUtility.sendPacket(player, new SM_MINION(activate));
	}
	...
	public void relocateDoping(Player player, int minionObjectId, int targetSlot, int destinationSlot) {
		...
		if (destinationSlot - 2 > scrollBag.length - 1) {
			...
			//PacketSendUtility.broadcastPacket(player, new SM_MINION(9, 0, minionObjectId, targetItem, targetSlot, destinationSlot), true);
			PacketSendUtility.sendPacket(player, new SM_MINION(9, 2, targetItem, destinationSlot));
			minions.getDopingBag().setItem(0, targetSlot);
			PacketSendUtility.sendPacket(player, new SM_MINION(9, 2, targetItem, targetSlot));
		} else {
			minions.getDopingBag().setItem(scrollBag[destinationSlot - 2], targetSlot);
			PacketSendUtility.sendPacket(player, new SM_MINION(9, 2, scrollBag[destinationSlot - 2], targetSlot));
			minions.getDopingBag().setItem(targetItem, destinationSlot);
			PacketSendUtility.sendPacket(player, new SM_MINION(9, 2, targetItem, destinationSlot));
		}
	}
	...
	public void addItemToDopingBag(Player player, int action, int minionObjectId, int itemId, int targetSlot) {
		...
		PacketSendUtility.sendPacket(player, new SM_MINION(9, 0, itemId, targetSlot));
	}
	...
	public void buffPlayer(final Player player, final int minionObjectId, int itemId, final int slot) {
		...
		if (player.isItemUseDisabled(limit)) {
			...
			ThreadPoolManager.getInstance().schedule(new Runnable() {
				@Override
				public void run() {
					PacketSendUtility.sendPacket(player, new SM_MINION(9, 3, useItemId, slot));
				}
			}, useDelay);
			return;
		...
		PacketSendUtility.sendPacket(player, new SM_MINION(9, 3, itemId, slot));
	...
	public static MinionService getInstance() {
		return MinionServiceHolder.INSTANCE;
	}
	
	private static class MinionServiceHolder {
		private static final MinionService INSTANCE = new MinionService();
	}
}