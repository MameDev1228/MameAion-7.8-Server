...
public class Item extends AionObject implements IExpirable, StatOwner {
	...
	public static int MAX_BASIC_STONES = 3;
	...
	//Tuning 6.x
	private Map<Integer, ItemRndBonus> rndBonus = new FastMap<Integer, ItemRndBonus>();
	private RndBonusEffect rndBonusEffect;
	...
	public Item(int objId, ItemTemplate itemTemplate) {
		...
		grindSocket = 0;
		grindColor = 0;
		ItemCustomSetTeamplate itemPreSettingTemplate = DataManager.ITEM_CUSTOM_SET_DATA.getCustomTemplate(itemTemplate.getTemplateId());
		if (itemPreSettingTemplate != null) {
            this.enchantLevel = itemPreSettingTemplate.getEnchantLevel();
            this.authorizeLevel = itemPreSettingTemplate.getAuthorizeLevel();
			if (itemPreSettingTemplate.getManaStone() != null && itemPreSettingTemplate.getManaStone().size() > 0) {
                for (int i = 0; i < itemPreSettingTemplate.getManaStone().size(); i++) {
                    ItemSocketService.addManaStone(this, itemPreSettingTemplate.getManaStone().get(i), i);
                }
            }
        }
		...
		updateChargeInfo(0);
		this.rndBonusEffect = new RndBonusEffect(this);
	}
	...
	public Item(int objId, int itemId, long itemCount, int itemColor,
		...
		int amplificationSkill, int SkinSkill, boolean lunaReskin, int reuctionLevel, int unSeal, boolean isEnhance, int enhanceSkillId, int enhanceEnchantLevel, int grindSocket, int grindColor, boolean contaminated, long grindStone, int grindSlot) {
		super(objId);
		...
		DAOManager.getDAO(InventoryDAO.class).loadItemRndBonus(this);
	...
	/**
	 * @return the echantLevel pve and pvp
	 */
	public int getEnchantPvPvELevel() {
		if (this.getItemTemplate().getEnchantType() == EnchantType.PVP) {
	...
	/**
	 *  the echantLevel pve and pvp level to set
	 */
	public void setEnchantPvPvELevel(int level) {
		if (this.getItemTemplate().getEnchantType() == EnchantType.PVP) {
	...
	public int getSockets(boolean isFusionItem) {
        ...
        if (itemTemplate.isWeapon() || itemTemplate.isArmor()) {
            if (isFusionItem) {
                ...
                numSockets = fusionedItemTemplate.getManastoneSlots();
	...
	public RndBonusEffect getRndBonusEffect() {
		return rndBonusEffect;
	}
	...
	public Map<Integer, ItemRndBonus> getRndBonus() {
		return rndBonus;
	}

	public void setRndBonus(Map<Integer, ItemRndBonus> rndBonus) {
		this.rndBonus = rndBonus;
	}
}