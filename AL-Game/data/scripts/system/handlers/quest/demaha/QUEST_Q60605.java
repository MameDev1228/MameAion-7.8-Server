...
public class QUEST_Q60605 extends QuestHandler
{
	private final static int questId = 60605;
	private final static int[] npcs = {806978, 731925, 807006};
	...
	public void register() {
		...
		qe.registerQuestItem(182216756, questId);
		qe.registerQuestItem(182216757, questId);
		qe.registerQuestItem(182216758, questId);
		qe.registerQuestItem(182216759, questId);
		qe.registerOnEnterZoneMissionEnd(questId);
		qe.registerOnEnterZone(ZoneName.get("LDF8_D_ITEMUSEAREA_Q60605A"), questId);
    ...
    public boolean onLvlUpEvent(QuestEnv env) {
        return defaultOnLvlUpEvent(env, 60604, true);
    }
    ...
    public HandlerResult onItemUseEvent(final QuestEnv env, Item item) {
        ...
        if (id == 182216756) {
            if (var == 1 && player.isInsideZone(ZoneName.get("LDF8_D_ITEMUSEAREA_Q60605A"))) {
				qs.setQuestVar(2);
				updateQuestStatus(env);
				removeQuestItem(env, 182216756, 1);
				...
				QuestService.addNewSpawn(800060000, 1, 703728, player.getX(), player.getY(), player.getZ(), (byte) 0);
				return HandlerResult.SUCCESS;
            }
        } else if (id == 182216757) {
            if (var == 4 && player.isInsideZone(ZoneName.get("IDF8_MISSION03_ITEMUSEAREA_Q60605B"))) {
				qs.setQuestVar(5);
				updateQuestStatus(env);
				giveQuestItem(env, 182216758, 1);
				removeQuestItem(env, 182216757, 1);
				...
				QuestService.addNewSpawn(302580000, 1, 703728, player.getX(), player.getY(), player.getZ(), (byte) 0);
				return HandlerResult.SUCCESS;
			}
        } else if (id == 182216758) {
            if (var == 7 && player.isInsideZone(ZoneName.get("IDF8_MISSION03_ITEMUSEAREA_Q60605C"))) {
				qs.setQuestVar(8);
				updateQuestStatus(env);
				giveQuestItem(env, 182216759, 1);
				removeQuestItem(env, 182216758, 1);
        ...
        } else if (id == 182216759) {
            if (var == 10 && player.isInsideZone(ZoneName.get("IDF8_MISSION03_ITEMUSEAREA_Q60605D"))) {
				qs.setQuestVar(11);
				updateQuestStatus(env);
				giveQuestItem(env, 182216760, 1);
				removeQuestItem(env, 182216759, 1);
        ...
        if (qs != null && qs.getStatus() == QuestStatus.START) {
			...
			if (zoneName == ZoneName.get("IDF8_MISSION_03_Q60605_302580000")) {
				...
				if (var == 11) {
					...
					removeQuestItem(env, 182216760, 1);
					return true;
				}
			}
		}
