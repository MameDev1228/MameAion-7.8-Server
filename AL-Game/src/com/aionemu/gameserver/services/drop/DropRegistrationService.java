...
import com.aionemu.gameserver.services.MameFieldRewardService;
...
public class DropRegistrationService
{
	...
	public void registerDrop(Npc npc, Player player, int heighestLevel, Collection<Player> groupMembers) {
		...
		if (player.isInGroup2() || player.isInAlliance2()) {
			...
			groupMembers = player.getPlayerGroup2().getMembers();
			switch (lootGrouRules.getLootRule()) {
				case ROUNDROBIN:
					int size = 0;
					if(player.isInGroup2()) {
						size = player.getPlayerGroup2().size();
					} else {
						player.getPlayerAlliance2().size();
					}
				...
				break;
				case FREEFORALL:
					winningPlayers = player.getPlayerGroup2().getMembers();
				break;
				case LEADER:
		...
		for (Player p : dropPlayers) {
			...
			PacketSendUtility.sendPacket(player, new SM_MINION(true, npcObjId));
			Set<DropItem> drops = getCurrentDropMap().get(npcObjId);
			if (drops == null || drops.size() == 0) {
				npc.getController().onDelete();
			} else {
			...
			PacketSendUtility.sendPacket(player, new SM_MINION(false, npcObjId));
			if (drops == null || drops.size() == 0) {
				return;
			}
		}
