...
@AIName("DF4_3011_Boss_Da_1")
public class DF4_3011_Boss_Da_1AI2 extends AggressiveNpcAI2
{
	...
	private void checkTurningTide() {
		checkTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new Runnable() {
			@Override
			public void run() {
				Npc siegeGate1 = getPosition().getWorldMapInstance().getNpc(257793);
				Npc siegeGate2 = getPosition().getWorldMapInstance().getNpc(257840);
	...
	private void conquerorPassion() {
		SkillEngine.getInstance().getSkill(getOwner(), 20665, 60, getOwner()).useNoAnimationSkill();
	}
