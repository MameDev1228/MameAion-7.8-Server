...
public class LifeStatsRestoreService
{
	...
	private static final int DEFAULT_FP_REDUCE_DELAY = 2000;
	private static final int DEFAULT_FP_RESTORE_DELAY = 2000;
	...
	public Future<?> scheduleFpReduceTask(final PlayerLifeStats lifeStats, Integer costFp) {
		return ThreadPoolManager.getInstance().scheduleAtFixedRate(new FpReduceTask(lifeStats, costFp), 2000, DEFAULT_FP_REDUCE_DELAY);
	}
	public Future<?> scheduleFpRestoreTask(final PlayerLifeStats lifeStats) {
		return ThreadPoolManager.getInstance().scheduleAtFixedRate(new FpRestoreTask(lifeStats), 2000, DEFAULT_FP_REDUCE_DELAY);
	}
